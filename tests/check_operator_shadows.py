...
#
# As of the `override` operator, a global `def` / `def-persist` of an operator
# name is now a HARD RUNTIME error (`/invalid-name`) -- this lint is the fast
# static pre-check for it.  `override` is the sanctioned escape hatch (it
# REQUIRES an operator name, else `/undefined`), so an `override` binding is an
# explicit, intentional shadow and is NOT flagged.
#
...
# The sanctioned, explicit operator-shadow binder -- intentional, never flagged.
OVERRIDE_BINDERS = {"override"}
# All recognized binders (used to terminate the forward scan).
ALL_BINDERS = GLOBAL_BINDERS | LOCAL_BINDERS | OVERRIDE_BINDERS | {"def-prim", "def-str-prim", "store", "put"}
