...
# The 3 bestagon prototype tests are CPU-heavy and reliably exceed the 60s
# timeout on a debug build (-O0 + ASan/UBSan is 10-30x slower); they pass in
# well under the limit on the optimized build.  Skip them (with reason) on a
# debug build rather than reporting a spurious TIMEOUT; require them to PASS on
# the optimized build, where they are a real release gate.  Run the optimized
# suite with: TRIX_BIN=./trix.opt tests/run_all.sh
build_line=$("$BIN" --version 2>/dev/null | grep '^build:')
if [[ "$build_line" != *"optimized"* ]]; then
    perf_reason="perf test; times out on debug build (-O0+ASan), run on the optimized build via TRIX_BIN=./trix.opt"
    SKIP_REASON[test_bestagon_proto_ai_v2_perf.trx]="$perf_reason"
    SKIP_REASON[test_bestagon_proto_fog_perf.trx]="$perf_reason"
    SKIP_REASON[test_bestagon_proto_mapgen.trx]="$perf_reason"
fi
