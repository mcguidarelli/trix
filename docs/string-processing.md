...
\(    literal (             \)    literal )
\101  octal (1-3 digits)    \xFF  hex (exactly 2 digits)
...
```
(hello) 10 string copy            % => (hello\0\0\0\0\0)
```
...
```
[ 104b 101b 108b 108b 111b ]    % [h e l l o] as byte array
save
    dup 0 72b put                % change first byte
restore
0 get                            % => 104b (rolled back to 'h')
```
...
```
save
    (hello) dup 0 72b put        % allocated after save
restore
% string no longer exists (heap rolled back)
```
...
**`get-interval` on strings:** Allocates a new string in VM and copies the
requested byte range. Returns a fresh string, not a view.

### 16.4 String Copy Semantics

Strings are **not** copy-on-write. Every copy operation (`copy`, `concat`,
`get-interval`, `uppercase`, `lowercase`, etc.) allocates a new string in the
VM heap. The source string is never shared or aliased.
