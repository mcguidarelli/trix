...
#### Mutex constraints
  ...
  * **`$$$` prefix** and **`#$$$` suffix** raise `/syntax-error`
    (max two `$`).
...
\t    tab (9)               \\    backslash
\(    literal (             \)    literal )
\101  octal (1-3 digits)    \xFF  hex (2 digits)
...
within a (string)`).  In particular `\d`, `\w`, `\s`, `\b` and other
...
| 3.11 | Dict | `dict put get known? remove begin end def store` | Dict construction, lookup, dict-stack frames |
| 3.12 | Set | `set set-member? union intersection difference` | Hashable-element set operations |
...
| 3.18 | Modules | `import export module begin-module end-module` | Module-scoped name binding + load-time linkage |
| 3.19 | Formatting | `sprint-fmt print-fmt scan-fmt` | Printf-style format strings + parse-back |
| 3.20 | VM and memory | `save restore vm-status set-global vm-global-gc` | Save/restore transactions, global VM control, GC trigger |
...
| 3.24 | Interrupt system | `set-interrupt-handler raise-interrupt mask-interrupts` | Cooperative async-signal-style interrupts |
| 3.25 | Floating-point env | `fp-getround fp-setround fp-test-exception` | IEEE-754 rounding mode + exception flags |
| 3.26 | Lazy sequences | `iterate take drop force lazy-cons` | Cons-cell-style streams + thunks |
| 3.27 | Miscellaneous | `bind make-literal make-executable` | Attribute manipulation, PostScript-legacy ops |
| 3.28 | Pipeline | `pipeline pipeline-source pipeline-sink connect close-pipe` | Producer/consumer staged dataflow |
| 3.29 | Actors | `actor-spawn actor-send actor-recv actor-self actor-status` | Erlang-style isolated-state message-passing |
| 3.30 | Supervision | `supervisor restart-strategy which-children` | OTP-style child-process supervision trees |
| 3.31 | Logic / backtracking | `unify choice find-all aggregate naf once` | Prolog-style relational programming + cut |
| 3.32 | Reactive cells | `cell cell-set cell-watch cell-bind` | FRP-style reactive variables with dependency tracking |
| 3.33 | Coroutines | `coroutine yield resume coroutine-status` | Stackful coroutines (foundation for actors) |
| 3.34 | Protocols | `define-protocol extend-protocol protocol-implements?` | Open dispatch on type — Clojure-style protocols |
| 3.35 | Pattern matching | `match unify-match` | Destructuring matcher with binding capture |
| 3.36 | Transducers | `xf-map xf-filter xf-take transduce` | Composable collection transformations |
| 3.37 | GenServer | `genserver-call genserver-cast genserver-init` | OTP-style synchronous-message servers on top of actors |
| 3.38 | Closures | `closure make-closure closure-env` | First-class lexically-scoped functions |
| 3.39 | Contracts | `pre post invariant` | Design-by-contract assertions |
| 3.40 | Delimited continuations | `reset shift abort` | First-class continuations with scoped extent |
| 3.41 | Algebraic effects | `effect handle perform` | Effect handlers (algebraic-effects-style) |
...
Total user-facing ops: 828.  See the corresponding subsection
...
rem-quo         num1 num2 -- remainder quotient-int
...
is-dict is-stream is-mark is-null is-curry is-thunk is-record
...
make-executable     any -- any               % set executable attribute (rejects Tagged, Record, Coroutine, Address)
...
```
mark 16#CAFE#u 42 3.14d (>HIBd) pack % pack BE: uint16 + uint32 + uint8 + double
...
- `make-executable` on Tagged, Record, or Coroutine raises `type-check` (must remain literal).
...
default-handler     /name op str arr arr arr -- % fallback uncaught-error handler (see below)
```
...
`fill-rect`, `render`, `render-to`, `get-cell`, `is`.
...
Release builds (`./build.sh --release`) gate every op, status key, and
side-table out, so probes via `where` will return false. The runtime
cost is paid only when tracking is enabled.
...
<!-- doctest: skip (TRIX_HEAP_TRACKING-only ops; bestagon placeholder helpers) -->
```trix
clear-alloc-stats
...
breakpoint          --                       % enter interactive debugger (no-op if debugger inactive)
...
The `logic` convenience dict provides short names: `var`, `is-var`, `bound?`,
`unify`, `choose`, `fail`, `cut`, `aggregate`.
...
`coroutine-release` on a live coroutine raises `invalid-access`.
...
`xf-take`/`xf-scan` raise `/type-check` here.
...
### Dicts
```
systemdict protocoldict userdict errordict handlersdict
numbers classification environment
```
...
save
/sum 0 def
1 1 100 { sum add /sum exch def } for
sum 5050 eq (test: for 1..100 sum) rt-ok
restore clear

save
/count 0 def
5 { /count count 1 add def } repeat
...
```
save
(hello) dup 0 72 put    % changes 'h' to 'H'
restore
...
`dup` creates a shallow copy. For ExtValue types (Long, etc.), both Objects
share the same ExtValue offset. Arithmetic on one will free the shared ExtValue,
corrupting the other. The interpreter's `make_clone` handles this in name
lookups and proc execution, but be aware in operator implementations.
