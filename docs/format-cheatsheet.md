...
| Alt form | `#` | Add `0x` / `0o` / `0b` prefix on integers; force decimal point on floats (e.g. `1.` not `1`) |
...
| Precision | `.N` or `.{i}` | Floats: digits after decimal point.  Strings: max chars.  Integers: ignored |
...
| `#d` *(alt form)*         | Decimal with explicit `+`/`-` if signed; no prefix |
...
| `/scan-type-fail`        | Value parses but out of range for target type    |
...
| Argument id out of range                                          | `({2})` with two args (max id = 1) |
