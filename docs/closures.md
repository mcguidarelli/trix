...
closure modifies the closure's dict-scope copy during that invocation, but
since the dict is ReadOnly, the store targets userdict instead.  For
mutable state, use a Cell (`cell` / `cell-set` / `cell-get`) and capture
the cell reference.
...
prevents accidental mutation of captured bindings.  If you `def` a name
inside a closure body, it goes into the current writable dict (usually
userdict), not into the captured dict.  This prevents action-at-a-distance
bugs where calling a closure silently modifies shared state.
...
/prefix (>> ) def
{ prefix exch to-str concat } [/prefix] closure-capture /display-impl exch def
...
- **ReadOnly captured dict.** You cannot `def` into the captured dict from
  inside the closure body.  `def` targets the nearest writable dict on the
  dict stack (usually userdict).  Use `store` if you need to write to a
  specific dict.
