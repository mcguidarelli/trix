...
| 00100 | `Long` | 64 | i64 inline |  |
| 00101 | `ULong` | 64 | u64 inline |  |
| 00110 | `Address` | 32 | `vm_offset_t` inline | VM offset (heap pointer) |
| 00111 | `Real` | 32 | f32 inline | Default float literal type |
| 01000 | `Double` | 64 | f64 inline |  |
...
```trix
% Float to Integer (truncation, no rounding op needed)
3.7 floor   /integer-type reinterpret    % WRONG -- bit cast, not value cast
% Right way: roll through to-string / to-number
3.7 floor 32 string to-string to-number  % -- 3 (Integer)
% Or use the promoted-infix lattice (Real → Integer is lossy, no clean op)
...
## 8. Quick reference — what's where
  ...
  * **Where the lattice lives in source:** `src/ops_math.inl` —
