...
| CI | -- | `.github/workflows/` | {gcc-15, clang-20} x {Debug, Release} matrix, README example execution, doc-link integrity, clang-format |

`./runtests.sh` is the battery CI runs: in-process suite, standalone
sweep, golden, PTY, CLI matrix, snapshot, eqref-overflow, sandbox-mode,
and fuzz-regression replay, with a single summary table and a non-zero
exit on any failure.
...
5. **`tests/run_cli_tests.sh`** CLI matrix -- parse_args flags, exit
   codes, startup modes.
6. **Snapshot tests** -- orchestrated multi-phase snap-shot/thaw
   across separate invocations.
7. **eqref overflow** -- a separate invocation with
   `--test-eqgen-preload=4294967295`.
8. **Sandbox-mode tests** -- `--sandbox` host-op rejection.
9. **Fuzz-regression replay** -- every artifact in
...
Runs 149 test files inside **one** interpreter.  Structure:
...
- **ci.yml** -- the gate: {gcc-15, clang-20} x {Debug, Release} build
  matrix, each leg running `./runtests.sh`; then README example
  execution and doc link/anchor verification
  (`tests/check_doc_links.py`).  The build-and-test job also runs the
  documentation/fact gates (operator counts, error codes, per-op throws,
  operator shadows, doc-example execution, scanner-grammar, type-facts,
  collection-facts, reference-facts); plus a clang-format job (which also
  runs `cpp_style.py` and `assert_blank_line.py`) and a `signed-char`
  sign-conversion gate (full-feature compile under clang `-fsigned-char`).
- **clang-tidy.yml** -- curated static analysis over the single TU
  (checks in `.clang-tidy`).
- **codeql.yml** -- security analysis (push to master + PRs).
...
`fuzz/seeds/` (~465 files) teaches the fuzzer the language's shapes;
...
- **Shape over value** -- a seed exercising `${...}` under a tight
  `--vm-size` (GC mid-operation), extreme `for` bounds, forged
