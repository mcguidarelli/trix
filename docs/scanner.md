...
Trix supports six integer widths, two float widths, radix 2-36 notation, and
...
l       Long       64-bit   -9.2e18 to 9.2e18
ul      ULong      64-bit   0 to 1.8e19
...
**Why six integer widths?** Trix's 8-byte tagged value design stores small
types (Byte, Integer, UInteger, Real, Boolean) inline in the 4-byte value slot.
Larger types (Long, ULong, Double, Address) use an 8-byte `ExtValue` in VM
heap memory.  The suffix system lets the programmer choose the right width for
the task -- saving memory for small values while providing full 64-bit range
when needed.
...
(case-insensitive).  The trailing `#` is required.
...
**Design decision: Why `N#digits#` instead of `0x`/`0b`/`0o`?**

Most languages hardcode three radix prefixes: `0x` (hex), `0b` (binary), `0o`
(octal).  Trix's `N#digits#` notation is strictly more powerful:

| Notation       | Bases supported   | Consistency                          |
| -------------- | ----------------- | ------------------------------------ |
| `0x`/`0b`/`0o` | 3 (2, 8, 16)      | Three different prefixes to memorize |
| `N#digits#`    | 35 (2 through 36) | One pattern, any base                |

The `0x`/`0b`/`0o` notation would create two ways to write the same value
(`0xFF` vs `16#FF#`), which is worse than having one unfamiliar but consistent
syntax.  A programmer who learns `16#FF#` also knows `36#Z#`, `7#66#`, and
`2#1010#` -- something prefix notation can never provide.

The Trix audience is already learning stack-based notation, `def`, `exch`, and
reverse Polish order.  One more unfamiliar-but-superior syntax is no burden.
...
<(C:\Users\name)>     % Windows path (14 bytes, backslashes literal)
...
Arrays, dicts, and procedures are all constructed with mark-based
...
[]                     % empty array (minimum capacity 4)
```
...
[1 2 3]#r         % read-only array (put raises InvalidAccess)
...
<< /x 1 >>#r     % ReadOnly: def/store raises InvalidAccess
...
Strings, arrays, dicts, and procedures all share the `#` suffix delimiter
...
`is_numeric` (LC | UC | IN | FE) so `.` still flows through number parsing.
...
0xB6 [packed-idx]    % SystemExecValue for packed proc (2 bytes)
...
```
Format: 0xC6 [count: uint16] [element_type: uint8] [element data...]
```

All elements share the same type (encoded as the BinaryToken value for that
scalar type).  This is the most compact representation for numeric tables,
lookup arrays, and signal data.
...
| `Long` | 5 | 64-bit signed | SS = 1-3, then remaining bytes |
| `ULong` | 6 | 64-bit unsigned | SS = 1-3, then remaining bytes |
| `Address` | 7 | pointer-width | ExtValue in VM heap |
| `Real` | 8 | 32-bit float | 4 extra bytes |
| `Double` | 9 | 64-bit float | 8 extra bytes |
...
| `Name` | 15 | name by index | SS = 1-3 bytes |
...
also takes 2 bytes (header + 0xFF).  The maximum 3-byte payload covers
signed values to +/-8 million.  For larger values, the full `Long` or `ULong`
types are used.
...
| Error | Trigger |
| --- | --- |
| `SyntaxError` | Unbalanced delimiters (`(`, `)`, `<`, `>`, `[`, `]`, `{`, `}`), invalid escape sequence, odd hex digits in `<...>`, missing `>` in `<(...)>`, NUL or DEL in source, unterminated block comment, unterminated or empty local binding `\|...\|`, invalid or out-of-range `\|...\|#N` capacity suffix, `\u`/`\U` with insufficient hex digits or surrogate/out-of-range codepoint, empty `\{}` interpolation |
| `TypeError` | Invalid type in a context that requires a specific type |
| `LimitCheck` | Name length > 127, string length > 65,535, procedure nesting > 64, infix expression nesting > 64, infix expression token count > 128, field-access chain length > 64, =string/=array/=proc capacity exceeded |
...
### 11.1 Why `N#digits#` Over `0x`/`0b`/`0o`

Adding `0x`/`0b`/`0o` would create two syntaxes for the same three bases while
leaving the other 32 bases unreachable.  The `N#digits#` form is one pattern
for all 35 bases.  The Trix audience learns stack-based notation anyway --
one more unfamiliar syntax is no burden, especially when it's strictly superior.
...
suffix applies to strings, arrays, dicts, and procedures uniformly.
...
#### Mutex constraints
  ...
  * **`$$$` prefix** and **`#$$$` suffix** raise `/syntax-error` (max two `$`).
