...
Numbers, booleans, marks, dicts, and saves have the `IgnoresExecute` attribute.
...
| **Curry**    | push    | dispatch value+callable | dispatch value+callable   | dispatch value+callable  |

**Key observations:**

- **Array/Packed** are the only types where proc-body behavior differs from
