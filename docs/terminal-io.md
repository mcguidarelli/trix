...
| --------------------------- | ---------------------------------------------------------------- |
| 0x20 (SPACE)                | `/space` (spelled out, not a literal space-name)                 |
| 0x21..0x7E printable        | `/<char>` (the byte literally — `/a`, `/Q`, `/!`, `/~`)          |
...
a few cells per frame (a scrolling map, a live HUD, a falling-block playfield)
...
The full `screen-*` surface (15 ops, OpaqueHandle-backed):
...
| `screen?` | `any -- bool` | True iff the operand is a screen handle |
...
| SGR attrs | `bold`, `dim`, `italic`, `underline`, `blink`, `inverse`, `strike`, `reset-attrs` |
...
[ /bold /inverse ] { 18 5 (ALERT) print-at } with-attrs
...
want to fall back to a default canvas size should check `stdin
interactive?` (or wrap `terminal-size` in a `try-catch`).
