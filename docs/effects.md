...
| ----------------- | ------------------------- | -------------------------------------------- |
| `handle-effect`   | `handler-dict body --`    | `ops_effect.inl:73`                          |
| `perform`         | `...args /name -- result` | `ops_effect.inl:97`                          |
| `@effect-barrier` | (internal)                | `ops_effect.inl:65`, enum at `enums.inl:264` |
...
1. `perform`'s find-handler scan loop (`ops_effect.inl:108-128`): each
   barrier crossing decrements `err_scan` by 1; `@handler-scope`
   decrements by 2.
2. `perform`'s capture-rewrite-count scan loop
   (`ops_effect.inl:156-184`): increments `err_count` by 1 per
   `@effect-barrier` in the captured range.
3. `try_catch_handler`'s unwind switch (`ops_system.inl:1798-1808`):
   pops one entry per `@effect-barrier` crossed.
...
`ops_effect.inl:187-190`.  Rationale: avoiding per-captured-barrier
...
is invoked.  See `ops_effect.inl:180-181` and `:209-218`.
...
`ops_effect.inl:225-271`.

When a new barrier type with saved depths is added, BOTH the count loop
(`:156-184`) AND the rewrite-table emission loop (`:225-271`) need to
recognize it.
