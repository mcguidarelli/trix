...
Every value carries its type, access mode, and execution attribute in a single
byte, leaving 7 bytes for the value itself. This design enables:
...
Three independent properties are encoded in a single byte:
...
The remaining 7 bytes hold the value itself. Small values (Byte, Integer,
...
| `VerifyHasLength`        | Name, Array, Packed, String, Dict, Set                              |
...
```
% Division: second operand must be a non-zero finite number
...
{ 1 (hello) add } try pop
last-error-message
% => "operand #1 type mismatch: expected Numbers, actual String"

{ [1 2 3] 0 99 put } try pop
last-error-message
...
| Double   | `std::isfinite()` result check          | `1.8e308d 2.0d mul` => inf                   |
...
| 4   | Either is UInt128                | UInt128 (Int128 -> UInt128 sign-loss accepted)     |
...
Every user-visible type has a corresponding `is-*` predicate:
...
2. Curry objects compare both the captured value and the callable
3. Thunk objects compare state, proc, and cached result
4. All other types use `eq` semantics
5. Recursion depth is limited to 64 (raises `/limit-check` if exceeded)
6. NaN != NaN even in deep-eq (IEEE 754 semantics)
...
3 array dup 0 42 put readonly   % now read-only
...
```
% systemdict is read-only: built-in names cannot be redefined
{ /add 42 def } try             % => /read-only (systemdict is active)
