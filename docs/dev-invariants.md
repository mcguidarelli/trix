...
`m_widevalue_active_count` should be 0 when the VM is idle (no objects
on any stack).  A non-zero count at shutdown indicates a leak; both are
exposed to gray-box tests as `:status:extvalue-active` /
`:status:widevalue-active` (with `extvalue-free` / `widevalue-free` and
`-validate` companions).  `ExtValue::valid()` / `WideValue::valid()`
check the dead sentinel for freed-value access.
