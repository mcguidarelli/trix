...
/abs //dup { 0 lt { 0 exch sub } if } compose def
-5 abs              % => 5
```
...
% db-url thunk was never forced (overridden or not present)
```
...
/make-event-system {
    ...
    /on { |event callback|
        ...
        { handlers event get callback append }
        { [ callback ] }
        if-else
        handlers exch event exch put
    } def
...
/user-name (Alice) def
{ pop user-name (: logged in) append = } /login on
...
Unevaluated --force--> Evaluating --proc completes--> Evaluated
                          |    |                          |
                          |    +--re-entrant force-->  undefined-result error
                          |       (circular; state stays Evaluating)
                          |
...
Note the two distinct error edges: a **circular** force (forcing a thunk while
it is already evaluating) raises `undefined-result` and leaves the sentinel in
place, whereas a **proc error** resets the thunk to Unevaluated so the next
force re-runs the proc (see §6.7, §12.6).
