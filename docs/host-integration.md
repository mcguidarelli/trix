...
| --- | --- | --- |
| `stream` | `str byte -- stream` | Open file stream (modes: r, w, a, e, R) |
...
| `to-binary-token` | `scalar -- str` | Encode value as compact binary token                  |
...
/l0-interrupt { (Level0 interrupt!) = } override
/l1-interrupt { (Level1 interrupt!) = } override
```
...
interrupts-pending  % => int (bitmask of pending user IRQs: Level0=1, Level1=4, Level2=64)
...
static uint8_t vm_memory[256 * 1024];  // 256KB (MinVmSize), no malloc
...
line via `parse_args` (see [Command-Line Argument Parsing](#command-line-argument-parsing)).
...
| `-v`, `--version` | Print `trix MAJOR.MINOR.PATCH — the cat in concatenative` followed by a `build:`/`features:` line (two lines) |
