...
/validate-shape {
    dup tag-name valid-shapes exch set-member?
    { (invalid shape) assert } if not
} def
```
...
/register {
    ...
    /email exch def
    validate-username
    /ok { email { create-user } try-result } tag-bind
} def
...
Tagged values are allocated via `vm_alloc_n_ptr<Object>(2)`, which bumps the
...
offset table is part of the serialized VM state (`wellknown_offsets` in the
