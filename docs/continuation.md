...
Header is 12 bytes (six `length_t` = `uint16_t`).  Objects are stored
...
| `m_exec_rewrite_count` + exec-rewrite array | EXEC segment | `@try-rollback`, `@try-result-complete` (saved-depth at scan-1; try/try-result families); `@array/@packed/@string/@dict/@set/@record-for-all` (saved-depth at scan-1; for-all family, via `operator_is_forall_family()`); `@naf-barrier`, `@choice-count-barrier`, `@for-each-solution-barrier`, `@aggregate-barrier`, `@aggregate-reduce-barrier` (op_depth at scan-2; logic-programming family) |
