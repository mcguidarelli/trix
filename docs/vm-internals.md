...
Copyright 2026 Mark Guidarelli. Apache License 2.0.
...
Only three functions may modify the allocation watermark. This invariant
simplifies reasoning about heap state:
...
trx->vm_trim_alloc(reinterpret_cast<vm_t *>(ptr) + actual * sizeof(Object), wasted);
```
...
`snap_shot_op` function validates `m_vm_temp_ptr == m_vm_limit` before
serializing.
...
Like `valid_offset` but checks against `m_vm_ptr` rather than `m_vm_limit`,
ensuring the offset is within the committed (allocated) portion of the heap.

### Nulloffset Sentinel

`nulloffset` is defined as `vm_offset_t{0}`. Offset 0 is never a valid
allocation because the first byte of the heap is reserved (alignment padding
ensures all allocations start at offset > 0).
...
union {
    struct {
        ...
        uint32_t m_free_sentinel;  // (bytes 4-7) 0xDEADBEEF when free, 0 when in-use
    } s;
...
bytes 4-7 hold the sentinel `0xDEADBEEF`. When allocated, the entire 8 bytes
...
1. Validate the ExtValue is live (`m_free_sentinel != 0xDEADBEEF`). Catches
   double-free.
...
3. Push onto the free list at the specified save level: set `m_next` to old
   head, set `m_free_sentinel = 0xDEADBEEF`, update head.
...
vm_offset_t m_binding;         // 4 bytes: cached pointer to Dict::DictEntry::m_value
hash_t m_hash;                 // 4 bytes: wyhash (32-bit output)
length_t m_length;             // 2 bytes: string length
save_level_t m_binding_level;  // 1 byte: level when binding was set
vm_t m_data[1];                // flexible: string data (no nul terminator)
```
...
Each Name has a one-entry binding cache (`m_binding`, `m_binding_level`) that
accelerates dictionary lookup:

- **`m_binding`** stores a `vm_offset_t` pointing directly to a
  `Dict::DictEntry::m_value` field.
- **`m_binding_level`** records the save level when the binding was established.

**Fast path in `execute_name()`:** If `m_binding != nulloffset` and the
current save level has not invalidated the cache, the interpreter
dereferences the cached offset directly -- O(1) with no hash computation
or dict stack traversal.

**Safety:** Dict::DictEntry objects are **never relocated** (no rehashing, no
compaction). Once a binding is established, the vm_offset_t remains valid
for the lifetime of the entry. Only save/restore can invalidate it.

**Invalidation:** During `Name::restore()`, any binding with
`m_binding_level > restored_level` is cleared. This forces the next lookup
to walk the dict stack and re-establish the binding.
...
`ReadWriteFixed` (access byte `0x40`)
    Insert/update allowed. Capacity fixed at creation.

`ReadWriteDynamic` (access byte `0x60`)
...
**`Dict::name_lookup(trx, key)`** -- Full dict stack lookup:
...
- **`create_or_recycle(trx, maxlength)`:** Checks the pool before allocating new.
  ...
  entry pool).
- **`recycle(trx, offset, maxlength)`:** Returns a dict to the pool after use.
...
`Dict::create_temp(trx, maxlength)` allocates from the top-end temporary
...
`status_lookup()` uses a single `m_status_scratch` member to return computed
...
```cpp
m_status_scratch = *(m_op_ptr - index);   // raw copy -- aliases ExtValue
```
...
**Anti-pattern:** Storing `m_status_scratch` directly on a stack without
...
| `m_name_bucket_count`  | count              | 2053                    | Name hash table buckets      |
...
`m_status_scratch` and cloned on push (see [Status Probe Scratch Alias
...
9. **Double-free detection:** `0xDEADBEEF` sentinel catches double-free at
   runtime.
...
17. **Binding cache validity:** `Name::m_binding` is valid only when
    `m_binding_level <= m_curr_save_level`. Restore clears stale bindings.
