...
| Section | Contents |
| --- | --- |
| Identity | Magic `{'T','R','I','X'}`, `snapshot_version` (`uint32_t`, currently 178), endianness flag |
...
| Root Objects | A single `root_objects_offset` pointing to the heap-resident `Object[ROOT_OBJECT_COUNT]` array (11 entries: 4 eq-temporaries =string/=array/=dict/=set, 6 stdio/string/memory name strings, and the pending throw-with error data); the array itself lives in the VM blob, not the header |
...
| --- | --- | --- | --- | --- |
| `ExtValue` | 8 bytes | Long, ULong, Double, Address | `m_extvalue_free_list` | per-cell `m_extvalue_save_level` (in blob) |
| `WideValue` | 16 bytes | Int128, UInt128 | `m_widevalue_free_list` | per-cell `m_extvalue_save_level` (shared, in blob) |

The free-list **heads** (`m_extvalue_free_list` / `m_widevalue_free_list`) are
`Trix` fields serialized in the header (as `extvalue_free_list_offset` /
`widevalue_free_list_offset`); the save level is journaled per-cell in the VM
blob (the per-Object `m_extvalue_save_level`, shared by both kinds), not in the
header, so thaw can
...
The SnapShotHeader stores a single monotonic `snapshot_version` (currently
**178**) -- there is no major/minor/patch version. Thaw requires an **exact**
match: any difference (older *or* newer) is rejected with `/invalid-image-file`
before the name table is ever consulted.

```cpp
if ((std::memcmp(h.magic, MAGIC, 4) != 0) ||
    (h.snapshot_version != SNAPSHOT_VERSION)) {
    error(Error::InvalidImageFile, "thaw: invalid snap-shot file (bad magic or version)");
}
```

There is no version tiering, no forward-compatibility "best effort", and no
"accepted with caution" path: a snapshot is loadable only by the exact build
that wrote it. A separate FNV-1a `operator_table_signature` check likewise
rejects cross-build images. Because any version delta is rejected outright, the
name-table-growth and newer-image `/undefined` scenarios cannot occur -- thaw
fails fast with `/invalid-image-file` first.
