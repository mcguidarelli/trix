...
```
% Type-correct but domain-wrong:
-1 sqrt              % NaN -- no type error, but meaningless
[] 0 get             % range error -- empty array
0 div                % div-by-zero
```
...
/create-user {
    ...
    3 -1 roll dup length 0 gt precondition  % name non-empty
    exch dup 0 gt precondition              % age positive
    dup 150 lt precondition                 % age reasonable
    exch dup length 0 gt precondition       % email non-empty
