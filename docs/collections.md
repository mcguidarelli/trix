...
- **Dict**: not supported (identity only via `eq`)
...
arr 1 3 get-interval       % sub-array [1..2]
...
[1 2 3] { } bind % bind resolves names to operators
```
...
printf/scanf formatting, binary pack/unpack, base64 encode/decode.
...
wyhash keys, prime bucket counts (table up to 65537).
...
**VM layout:** RecordSchema (shared, uint32_t field_count + Name[]) and
RecordInstance (vm_offset_t schema + Object[] fields). See docs/record.md
...
seed { pred } { emit state } lazy-unfold
head { tail-thunk } lazy-seq  % manual construction
...
| 1-9   | Primitives | 2-5   | SS = value byte count                |
...
| 13    | Reserved   | --    | 1 free slot (slot 31 also unused)    |
...
| --------------- | ------------------------------- | ---------------------------------------------- |
| PushOpDirect    | pushed to op stack in proc body | most (not Operator, String)                    |
| IgnoresExecute  | pushed even via executable name | numbers, Bool, Mark, Dict, Set, Tagged, Record |
| HasObjectAccess | R/W access control on Object    | Array, Packed, String                          |
...
| Dict   | O(1) hash put   | O(N) rehash on grow         |
| Set    | O(1) hash add   | O(N) rehash on grow         |
