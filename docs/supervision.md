...
All three are implemented in C++ within `src/ops_supervision.inl` (1803
...
supervisor_op (runs on caller's coroutine):
  ...
  5. Push [mark, state_offset, proc] on caller's op stack
...
First tick:
  @supervisor-init runs:
    - pops state_offset from op stack
...
2. If tagged with `/down`: extract payload dict, call `supervisor_handle_down`
3. If any other message: discard (free ExtValue if applicable)
4. Repeat until mailbox empty
5. Block: set `m_blocked_reader`, push `@supervisor-check`, sleep

This is simpler than `actor-recv-match` because the supervisor only cares
about `/down` messages.  The cooperative scheduler guarantees no new
messages arrive during processing (the supervisor holds the CPU until it
blocks).
...
and wakes it (via `supervision_deliver_message` which calls
`supervision_wake`).
...
| File                         | Content                                        |
| ---------------------------- | ---------------------------------------------- |
| `src/ops_supervision.inl`    | All supervision operators (~1803 lines)        |
