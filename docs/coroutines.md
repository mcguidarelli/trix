...
Trix coroutines are cooperative, stackful, round-robin-scheduled execution
contexts that share a single VM heap.  Each coroutine has its own operand,
...
- **Deterministic scheduling**.  Round-robin with explicit yield points means
  no data races, no locks, no atomics.  The programmer controls exactly where
  interleaving occurs.
...
The scheduler maintains a circular singly-linked list of CoroutineContexts
using the `m_next` field.  Member variables track the scheduler state:

<!-- doctest: skip (scheduler member-variable listing, not runnable code) -->
```
m_coroutine_head        head of scheduler queue (circular linked list)
...
Main (coroutine #0) is always in the list and is never removed.  Its
...
```
1. Remove dead coroutines from the linked list
2. Walk the circular list from current->m_next:
   a. If a coroutine is Ready: switch to it, return
   b. If a coroutine is Sleeping and wake_time <= now: switch to it, return
   c. Otherwise: skip, continue walking
3. Check if the current coroutine itself is re-eligible
4. If all coroutines are sleeping:
   a. Find the earliest wake_time across all sleeping coroutines
   b. Sleep the real OS thread until that time (nanosleep)
   c. Walk the list again to find the newly woken coroutine
5. If no runnable coroutines exist: reload main
```

The scheduler is O(n) in the number of coroutines per scheduling decision.
For typical workloads (< 50 coroutines), this is negligible compared to the
cost of the work each coroutine performs between yields.
...
leaving the stack empty, calling `coroutine-die`, being killed via
`coroutine-kill`, or dying from an uncaught error.
...
**Capacity on default 256 KB VM**: After interpreter initialization
(~60 KB for systemdict, names, streams, etc.), roughly 200 KB remains.
At ~4.7 KB per coroutine, this supports ~40 simultaneous coroutines.
With the free list recycling stack blocks from dead coroutines, the
throughput over time is much higher.
...
2. Scheduler walk: O(n) linked list traversal
3. `coroutine_load`: 14 offset-to-pointer conversions + 2 stores

For n < 50 coroutines, the total cost is dominated by the pointer
conversions -- roughly 30 pointer arithmetic operations.  This is
comparable to a single dictionary lookup.
...
The scheduler is O(n) per scheduling decision where n is the number of
coroutines in the linked list (including dead ones not yet unlinked).
Dead coroutines are removed at the start of each scheduling pass.
...
| `src/dispatch.inl`         | Dispatch entries for 18 standard + 4 control operators        |
...
CAPACITY:     ~40 simultaneous on default 256KB VM
```
