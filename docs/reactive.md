...
All four are serialized in SnapShotHeader (PATCH 75).
...
```
my-cell { cell-get } curry computed
```
...
2. Push trampoline on exec stack:
   ```
   Exec: ... | prev_current_cell | cell_offset | @cell-eval | proc
   ```
...
is supported, limited by exec stack capacity (~500 levels on default
...
`cell-get`), but does NOT set `m_current_cell` -- so no dependency edge
is created for the calling context.
...
wired into the graph.  **Irreversible across `restore`** -- a disposed
cell is not resurrected by rolling back to a save level taken before the
dispose.
...
| `@batch-end` | Batch proc completes | Decrement `m_batch_depth`; if zero, sweep deferred array and fire watchers for cells with net change |
...
**Barrier integration**: `@cell-eval` and `@batch-end` are recognized
by `try_catch_handler` as exec stack barriers.  On error, they are
replaced with their fail variants, and the exec stack is cut back.

**Exec stack layout** (cell-get trampoline):

```
... | prev_current_cell_int | cell_offset_int | @cell-eval | proc
```

`@cell-eval` pops `cell_offset` and `prev_current_cell` (two integers
below the barrier on exec stack).  `@cell-eval-fail` does the same
before re-raising.
...
(secret123) password cell-set   % Form is valid
```
...
processed cell-get =         % -> 90 (recomputed: 10+30+40... depends on filter)
```
...
0.95d cpu-load cell-set         % ALERT: system unhealthy!
0.5d cpu-load cell-set          % (no alert -- healthy again)
```
...
### 9.6 No Equality-Based Skip

`cell-set` always triggers invalidation, even if the new value equals the
old value.  A future optimization could compare old and new values and
skip invalidation if equal.  This would require defining equality
semantics for all types (currently `cell-set` accepts any value).

### 9.7 Exec Stack Depth for Deep Chains

Each level of computed cell nesting during evaluation consumes 4 exec
stack entries.  A chain of 20 computed cells requires ~80 entries.  The
default exec stack (2048 entries) supports chains of ~500 cells.  Deeper
chains require a larger exec stack via construction-time Config.
...
### 10.3 The 700-Operator Runtime

| Category                                              | Operators |
| ----------------------------------------------------- | --------- |
| Core language (stack, math, string, flow, etc.)       | ~589      |
