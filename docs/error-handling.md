...
from Trix code. The 57 named error codes cover every failure mode the VM can
produce, and all 57 are throwable and catchable by user code.
...
Trix defines 57 error codes, each with a corresponding name for use in `throw`
...
```
Trix type-check --add--: add: expected Number, got String
```
...
### Why 57 error codes instead of error classes?
