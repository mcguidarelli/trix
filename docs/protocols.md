...
/Describable protocol-methods  % => [/describe /category]
...
is reachable from userdict.  When `def-method` adds an entry to the
...
| userdict entry (per method)     | 20 bytes (Dict entry)                     |
