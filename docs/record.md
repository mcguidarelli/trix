...
The returned procedure is a normal executable array `{ <schema_offset> @record-ctor }`.
...
```
RecordSchema {
    uint32_t  m_field_count;     // 4 bytes
...
Both structs use `uint32_t`/`vm_offset_t` headers to ensure 4-byte alignment
of the Object arrays that follow.
...
Records set `ReadOnlyAccess` in their `m_aat` byte. This means `put` and other
write operations are rejected at the access-check level, before any
record-specific logic runs. This is defense-in-depth: even if someone tries to
treat a record as an array or dict, the access check catches it.
...
Object attribute flags: `PushOpDirect | IgnoresExecute | HasObjectLength | UsesVM`.
...
Both use `uint32_t`/`vm_offset_t` as the first member to ensure 4-byte
alignment of the Object array. Allocations use `vm_alloc<RecordSchema>()` and
...
```
{ <schema_offset_as_uinteger> @record-ctor }
```

When this procedure executes:
1. The literal UInteger pushes `schema_offset` onto the operand stack.
2. `@record-ctor` (a control operator) pops it, reads `field_count` from the
   schema, pops N values, allocates a RecordInstance, and pushes the Record.
...
- `vmsize_op()` — returns `sizeof(vm_offset_t) + field_count * sizeof(Object)`
