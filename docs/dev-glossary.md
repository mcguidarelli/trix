...
of truth consumed by the error walk, `unwind_exec_to`, and
...
`#=` lexical suffix and the `=string`/`=array`/`=proc`/`=dict`/`=set`
ops (and `*-from-mark` collectors).  One buffer per kind, guarded by a
...
(`<= 64` bytes).  Four bins (32 / 40 / 48 / 56 bytes), exclusive
...
bounds) — using the *descriptor table*'s per-op metadata.
