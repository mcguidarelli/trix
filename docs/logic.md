...
new journaling infrastructure needed.
...
auto arr = payload.array_objects(trx);
auto curr_save_level = trx->m_curr_save_level;
if (arr[0].save_level() != curr_save_level) {
    Save::save_object(trx, &arr[0]);   // journal the old value
} else {
    arr[0].maybe_free_extvalue(trx);   // same save level: overwrite directly
}
arr[0] = value.make_copy(curr_save_level);        // store the bound value
```
...
2. Pops `index` and `alt_array` from exec stack
3. Pops save_object + index + alt_array from exec stack (3 slots)
4. The save level stays active (bindings persist)
5. Whatever the alternative left on the operand stack is the result
...
identical by binding unbound lvars.  Returns false on
mismatch (no variables bound on failure).
...
- A structured result (array, record, tagged, dict, string) survives
  ...
  `${...}`.  Global VM is journal-skipped, so `restore` does not roll it
  back.  See §8.3.
...
does not roll it back, and `find-all` collects the structured result:
...
| Control operators | 4 (@choice-barrier, @choice-fail, @find-all-barrier, @find-all-fail) |
