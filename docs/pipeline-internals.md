...
2. **Lazy sequences** -- demand-driven iteration (47 operators, thunk-based)
...
**VM allocation**: `PipeBufferHeader` + `Object[capacity]` in a single
contiguous region (two sequential `vm_alloc` calls, not necessarily
adjacent but both on the VM heap).
...
pipe_wake(trx, writer);   // sets status=Ready, wake_time=0
```
...
```cpp
if (pipe_is_lazy_seq(trx, root_source)) { ... }
else if (root_source.is_array() || root_source.is_packed()) { ... }
else { error(...); }
```
...
blocked on full buffers.  The terminal operator currently relies on
cascading close for cleanup.  Upstream stages blocked on `pipe-put` will
remain sleeping until the pipeline's coroutines are eventually cleaned up
by the scheduler (when the main coroutine exits or all coroutines die).

**Known limitation**: Defensive kill-all after the sink completes is not
yet implemented.  For normal completion, all stages die via cascading
close.  For error paths, zombie upstream stages are harmless but consume
memory until cleaned up.
...
Each coroutine has an exec stack of 128 slots.  The pipeline framework
uses 4-5 slots (try-barrier, cleanup, coroutine-complete, stage proc
frame).  The remaining ~123 slots are available for user proc execution.
...
Four stage types (map, filter, flat-map, tap) use Trix procs built
from C++.  Batch and take use the C++ control operator (@pipe-stage).
...
### 9.5 No Kill-All on Error (Current Limitation)

After a sink error (from upstream cascade), upstream stages that are
blocked on full buffers remain sleeping.  The terminal operator does not
currently perform a defensive kill-all.

**Rationale**: Normal completion always kills all stages via cascading
close.  Error paths leave zombie stages that are harmless (they consume
memory but never run) and are cleaned up when the interpreter exits.
Implementing kill-all requires storing coroutine handles across the
exec stack, which adds complexity to the terminal operators.
...
| `src/enums.inl` | SystemName entries: 22 standard ops + 4 control ops (PipePutRetry, PipeGetRetry, PipeStage, PipeCleanup) |
