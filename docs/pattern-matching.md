...
The cost is one extra array allocation (4 elements) per `when` call.
...
- `let`: One Dict allocation (40 + N*20 bytes).
- `destructure`: One Dict allocation (40 + N*20 bytes) + N value clones.
...
`match` passes the value to the body but also leaves the original below the
result, which complicates a recursive evaluator.
...
| `/stack-underflow` | Fewer stack values than names (`let`)                   |
...
- **match-all not implemented.** Only first-match semantics available.
