...
| Radix | `16#FF#` | `16 #FF#` is a scan error |
...
| Locals preamble | `\| a b \| #8` or `\| a b \| #+6` | `\| a b \| #8` → locals default-capacity, body starts at `#8` |
...
- `base`: decimal integer 2–36. Values outside this range raise `SyntaxError`.
- `digits`: characters in the specified base (`0`–`9`, `A`–`Z`/`a`–`z`). Case-insensitive.
- The final `#` is required but the suffix after it is optional (defaults to Integer).
...
| `d`             | Double | 64-bit double | ±∞ is allowed                       |
...
Four bare names are recognized as numeric literals by the scanner (via `systemdict`):
...
not scanner keywords). Negative infinity is produced with `inf neg` or `inf#d neg`.
...
See § 14 for the broader Global VM directive family.
...
| `\0`–`\377`           | 0–255           | Octal: 1–3 octal digits                                     |
...
decoration (no type suffixes, no quoting, no delimiters):
...
`#$$` (any pair raises `SyntaxError`).  See § 14 "Global VM Scoping" for
...
#[=|$|$$][l|x][r|w]     → String with given attributes
```
...
the Mark. Elements may be any hashable type (same as dict keys: all types except
Null), and a single set may contain keys of different types (heterogeneous).
...
VM buffer (640 bytes of packed data, allocated lazily on first use, reused for
subsequent expressions on the same stream).  The packed representation uses
2-5 bytes per token instead of 8 bytes per Object; 640 bytes is the provable
worst case for 128 tokens.  The `scanner()` function extracts packed tokens one
...
| EOF / whitespace / delimiter / syntax-error byte | `SyntaxError` | direct error return |
...
```
0xC6  <count: uint16 native>  <element_type: uint8>  <element_data ...>
```

`element_type` encodes the element type (BinaryToken values for the scalar types). All
elements have the same type; `element_data` contains `count` values packed sequentially.
...
| 2     | `Integer`                   | 1–3 byte signed Integer (sign-aware encoding)                  |
| 3     | `PackedExt`                 | 128-bit (Int128/UInt128) via subcode byte; X=0, SS=offset size |
| 4     | `UInteger`                  | 1–3 byte UInteger                                              |
| 5     | `Long`                      | 1–7 byte Long                                                  |
| 6     | `ULong`                     | 1–7 byte ULong                                                 |
| 7     | `Address`                   | ExtValue-stored Address                                        |
| 8     | `Real`                      | 4-byte Real                                                    |
| 9     | `Double`                    | 8-byte Double                                                  |
...
value types. Encoding: `[header][subcode][offset: SS+1 bytes]`. `X=0`
...
| 0x00       | Int128                                                 |
| 0x01       | UInt128                                                |
| 0x02..0xFF | Reserved for future fixed-size heap-backed value types |
...
| Error | When |
| --- | --- |
| `SyntaxError` | Invalid or unbalanced syntax: odd hex digits in `<…>`, missing `>` in `<(…)>`, unmatched `)` / `>` / `]` / `}`, invalid suffix letter, NUL or DEL in source, unterminated block comment `%{`, unterminated or empty local variable binding `\|...\|`, invalid or out-of-range `\|...\|#N` capacity suffix, `\u`/`\U` with insufficient hex digits or surrogate/out-of-range codepoint, mutually-exclusive suffixes both present (e.g. `#=$`, `#$=`, `#$$=`) |
| `LimitCheck` | Name length > 127; string or proc body > MaxStringLength; proc nesting > 64; =string/=array/=proc capacity exceeded |
...
| `Unsupported` | Illegal suffix combination (e.g. `#pw`); binary token in unassigned range (0xEE–0xFF) |
...
int_suffix      = 'b' | 'i' | 'u' | 'ui' | 'l' | 'ul' | 'a'
                | 'B' | 'I' | 'U' | 'UI' | 'L' | 'UL' | 'A' ;
                  (* case-insensitive; at most one suffix; 'ui' is explicit UInteger *)
...
base            = DD { DD } ;
                  (* decimal 2-36; out of range raises SyntaxError *)
...
escape_code     = 'a' | 'b' | 'e' | 'f' | 'n' | 'r' | 't' | 'v'
                | '(' | ')' | '\'
                | '^' caret_char
                | OD [OD [OD]]              (* octal: 1-3 digits, 0-377 *)
...
(* 0xEE-0xFF: reserved, raise Unsupported *)
