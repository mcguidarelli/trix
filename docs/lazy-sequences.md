...
Typical total: **56-72 bytes per lazy node.**
...
```
curry(proc, curry(source_tail, compose(Force, compose(Exch, LazyMap))))
```
...
5. `LazyMap` — calls lazy-map recursively on the forced tail with proc
...
These operators produce new lazy sequences from existing ones.  All are lazy —
they don't force any elements until the result sequence is consumed.
...
| Curry chain (varies)        | 16-32     | 1-2 curries capturing state |
| **Total per node**          | **56-72** | Depends on transformation   |

For a simple `lazy-from` (no transformation), each node is 56 bytes.
For `lazy-map` (one curry capturing the proc), each node is ~72 bytes.
