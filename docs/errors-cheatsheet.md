...
| `/undefined-case` | `case` / `switch` fell through with no match and no default arm; or a protocol method called on a type with no implementation | `case`, `tag-match`, protocol dispatch | Add a default arm / `def-default-method`; or pre-filter inputs |
| `/undefined-result` | A math op produced a result with no meaningful representation (not NaN-shaped) | `log` of negative, `sqrt` of negative without complex domain | Domain-check inputs; route to NaN path explicitly |
...
| `/invalid-stream` | Operation on a closed / freed stream | Any stream op after `close` | Track stream lifetime; reopen if needed |
...
| `/snap-shot-error` | Snapshot internal-state failure (mid-write or unexpected layout) | `snapshot`, `thaw` | Likely a build incompatibility (snapshot version) — see `docs/snapshot-thaw.md` |
...
| `/invalid-format-string` | Format string mis-specified (`{:z}`, unmatched `{`, etc.) | `print-fmt`, `sprint-fmt`, `scan-fmt` | Hand-fix the format string; see `docs/format-cheatsheet.md` |
| `/scan-duplicate-arg-id` | Same `{N}` positional id used twice in a format string | `print-fmt` and friends | Unique-numbered placeholders |
| `/scan-input-fail` | `scan-fmt` consumed fewer args than the format string requires | `scan-fmt` | Provide more input; or use a shorter format |
| `/scan-match-fail` | Literal text in `scan-fmt` did not match input | `scan-fmt` | Loosen the literal or pre-process input |
| `/scan-type-fail` | Parsed value out of range for target type | `scan-fmt` with `{:d}` on `1e20` | Use a wider target type or guard input |
| `/scan-type-mismatch` | Target type doesn't accept the conversion at all | `scan-fmt` writing a non-numeric to an integer slot | Fix the format spec |
...
| `/effect-not-handled` | `perform` reached the top level with no enclosing `handle` | Algebraic-effects `perform` | Wrap the call site in `handle { ... } { handlers }` |
