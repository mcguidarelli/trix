...
distinguishes actors from plain coroutines by checking whether the
CoroutineContext has a mailbox attached.
...
- Survival across `save` / `restore`: journal-skipped, reclaimed by
  mark-sweep GC after the owning actor dies
...
| --- | --- | --- |
| CoroutineContext | 232 | Global VM (`ChunkKind::CoroutineContext`); GC-reclaimed |
| Stacks block | 4,480 | 560 Objects (128 op + 256 exec + 16 err + 32 dict + 128 scratch); global VM (`ChunkKind::CoroutineStacks`) |
| MailboxHeader | 16 | Global VM (`ChunkKind::Mailbox`); GC-reclaimed |
...
whose CoroutineContext has `m_mailbox != nulloffset`.  Returns false for
all non-coroutine types and for plain (non-actor) coroutines.  Works on
dead actors (the mailbox field persists after death).
...
When the mailbox is full, the sender yields to the scheduler.  The first
blocked sender gets targeted wakeup (O(1)); additional blocked senders
fall back to polling via `0 coroutine-sleep` + retry.
...
```cpp
static void actor_wake(Trix *trx, vm_offset_t blocked_offset) {
    if (blocked_offset != nulloffset) {
        auto *ctx = trx->offset_to_ptr<CoroutineContext>(blocked_offset);
...
Setting `m_status = Ready` and `m_wake_time_ns = 0` makes the coroutine
immediately eligible for scheduling.  The next scheduler pass finds it
in O(1).
...
`actor-send` delivers a message, it calls `actor_wake(m_blocked_reader)`
...
offset of the first sender that blocked on a full mailbox.  When
`actor-recv` drains a message, it calls `actor_wake(m_blocked_sender)` and
clears the field.

If a second sender blocks while `m_blocked_sender` is already occupied,
the second sender falls back to polling: it sleeps with `wake_time_ns = 0`
(immediate re-schedule) and retries.  This handles the rare case of
multiple simultaneous senders blocked on the same full mailbox without
the complexity of a waiter queue.
...
actor-send:
  ...
  3. Flush, sleep (UINT64_MAX if targeted, 0 if overflow), yield
...
void coroutine_cleanup_mailbox(CoroutineContext *ctx) {
    ...
    // Wake blocked sender (will retry, find dead, silent-drop)
    if (mbx->m_blocked_sender != nulloffset) {
        actor_wake(trx, mbx->m_blocked_sender);
        mbx->m_blocked_sender = nulloffset;
    }
}
```

This walks queued messages and frees any ExtValue objects (Long, ULong,
Double, Address).  It then wakes any blocked sender, which will retry
`actor-send`, discover the target is dead, and silently drop.

The mailbox memory itself is not freed to a free list.  Save/restore or
VM reset reclaims it with the rest of the heap.

### 4.7 Scheduler Fix

A pre-existing bug was fixed during actor development:
`coroutine_schedule()` walks the circular coroutine list starting from
`m_running_coroutine`.  When the running coroutine dies (via
`@coroutine-complete`) and is removed from the list by
`coroutine_remove_dead()`, the walk start is no longer in the list,
causing an infinite loop.  Fixed by checking if start is Dead and using
`m_coroutine_head` instead.  This bug only manifested when an actor died
while the main context was sleeping (e.g., `50 coroutine-sleep`).
...
Tested with 5 stages, 1000 messages, capacity-16 mailboxes.  Messages
traverse the ring with backpressure at each stage.
...
### 6.4 Why Single-Slot Blocked Sender?

The mailbox stores one `m_blocked_sender` offset.  A full waiter queue
would handle multiple blocked senders more efficiently.

| Approach               | Common case (1 sender) | Rare case (N senders) | Complexity |
| ---------------------- | ---------------------- | --------------------- | ---------- |
| Single-slot + overflow | O(1) targeted          | O(N) polling          | Low        |
| Waiter queue           | O(1) targeted          | O(1) targeted         | Medium     |
| Pure polling           | O(coroutines)          | O(coroutines)         | Trivial    |

Single-slot was chosen because:
- Multiple senders simultaneously blocked on one full mailbox is rare
- The common case (one blocked sender at a time) is O(1)
- The degenerate case is self-correcting (consumer drains, senders cycle)
- No additional data structures or CoroutineContext fields needed

### 6.5 Why Single Contiguous Allocation?

PipeBufferHeader uses two allocations (header + data array).  MailboxHeader
uses one.
...
| `src/enums.inl` | 18 standard + 5 control SystemName entries |
...
| `src/ops_actor.inl` | All 18 standard + 5 control operator implementations + helpers |
| `src/ops_pipeline.inl` | m_mailbox initialization in stage/source creation |
| `src/dispatch.inl` | Dispatch entries for all 18 standard + 5 control operators |
