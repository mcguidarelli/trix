...
waits with `actor-recv-timeout` plus ref-matching:
...
unwraps them.  This keeps the protocol invisible and unforgeable --
a user sending a raw `[/gen-call ...]` array via `actor-send` would
be treated as an info message (the name comparison uses pre-interned
names for exact matching).
...
`actor-recv-timeout` plus ref-matching, cleaning up the pending ref when
...
2. Build startup proc: [spec-literal, init-proc, @gen-server-init]
...
| Component                                   | Size                                    |
| ------------------------------------------- | --------------------------------------- |
| Startup proc (3 elements)                   | 24 bytes                                |
...
| Per-server overhead                         | ~56 bytes (startup + frame)             |
