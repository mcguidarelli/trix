...
(`quit`, snapshot ops, `coroutine-kill`, image-related, etc.). The
...
recursions and halts only when `arr length 1 le` is true. The source
pane shows `> 18      { 0 }`.
...
- `quit`, `exit` — would skip the restore.
- The snapshot quartet, `restore`, `save` — recursion/state confusion.
- `coroutine-kill`, image / persistence ops — break the harness model.
...
Solution: a persistent sid → path cache (`_dbg-source-cache`) is
populated at file-open time into a global-VM Dict. The cache survives
stream close and any save/restore. On halt, the source pane looks up
the current PC's sid in the cache, reads the file from disk if not
yet line-split-cached, and renders the right pane content.
...
On `q`, the tail of the capture stream is read out *after*
`alt-screen-leave` and printed to the real terminal, so anything the
program emitted before its final newline still reaches the user.
...
- The `--inspect*` CLI flags are still parsed but error out before
  loading any code.
...
- `src/ops_snapshot.inl` — stack-snapshot ops.
- `src/save.inl` — breakpoint-Dict prune hook on restore.
...
In a release build, this entire surface is gone. The `breakpoint` op
referenced inline in user code still parses (the scanner doesn't gate
on TRIX_DEBUGGER), but at execution time it dispatches to a no-op
stub — your `breakpoint`-decorated code is safe to ship to production.
...
The C++ side exposes ~22 ops, all gated behind `#ifdef TRIX_DEBUGGER`:
...
| `proc-disasm`          | `proc -- array`    | Disassemble to `[sid line col name]` rows         |
...
- **`vm-global-gc` is manual.** Long-running debug sessions that
  expand the breakpoint Dict or the sid → path cache enough to
  trigger a Dict grow can accumulate garbage auxiliary pool blocks
  in global VM. The default GC walker doesn't yet mark them. Cost is
  bounded; manual `vm-global-gc` reclaims them.
...
**`./trix --inspect FILE` errors with `unknown option`.** You're on a
...
`restore`, `quit`, the snapshot quartet, `coroutine-kill`, etc.) and
