...
```trix
vm-global-gc                % -- int  (bytes reclaimed by the sweep)
```

The result is the reclaimed byte count; `pop` it to discard when you
don't need the figure.
...
<<
  ...
  /live-blocks   int        % reachable blocks
  /live-bytes    int        % reachable bytes
  /free-blocks   int        % free-list blocks
  /free-bytes    int        % free-list bytes
  /by-kind       <<
...
* **Leaves** (no Object references in payload; walker is a bare
  ...
  `UInt128`, `String`, `Name`.  (`Name` is a leaf by rationale:
  its `m_data` are name bytes, `m_next` is covered by the
  Name-table bucket roots, and `m_binding` is a non-rooting
  `DictEntry`-slot cache that is intentionally NOT followed.)
...
* **Retired or reserved** (`Other` is a SENTINEL ONLY -- never a valid
  `gvm_alloc` tag, which asserts against it; the rest are RETIRED stream
  ChunkKinds or RESERVED for future migrations): `Other`, `Stream`,
  `Continuation`, `Screen`.  (`Cell` is **active** -- reactive cells are
  region-aware and walked by `cell_gc_walk`, so its `/cell` histogram
  bucket reports a non-zero count.)
...
4. **Bump from `m_vm_global`**: if free lists are empty, decrement
   ...
   the top of the global region.  Validates the bumped-down address against `m_vm_ptr` (the local main
   watermark); raises `Error::VMFull` if it would collide with local.
...
4. **Work-queue drain**: pop blocks off `m_gc_work_head`, dispatch by
   ...
   `gc_mark_object` on every reachable Object.  The drain order is a
   depth-first walk (the work queue is a LIFO stack, see below);
   traversal order is correctness-irrelevant by design.  The
   intrusive `m_next_in_work` link guarantees enough storage for any
   live set by construction (one link per block, no fixed-size
   scratch).

The work queue is an **intrusive singly-linked list (LIFO stack)** through
...
`m_vm_global` up to `m_vm_limit`.  For each block:

* If `m_kind == Free`, skip (already on free list).
* If `m_mark_gen == m_gc_current_gen`, the block is live; skip.
* Otherwise, the block is unreachable; call `gvm_free` to
