...
in Trix: 31 types, 828 operators, and a set of design choices that make
...
Trix provides 37 eager collection operators that work polymorphically on
...
```mermaid
flowchart LR
    subgraph "Eager Pipeline (37 ops)"
        direction LR
...
function composition, an 85-operator collection pipeline (37 eager + 48 lazy)
...
@typed-for (e.g., @integer-for):
  ...
  Overflow detection: signed types raise /numerical-overflow.
```
