...
/factorial { dup 1 le { } { dup 1 sub factorial mul } if-else } def
...
```
/factorial {
    dup 1 le { } { dup 1 sub factorial mul } if-else
} def
% mul happens AFTER factorial returns -> not tail position
```
...
- **The @call frame is for backtrace identification.** Operators are
  self-identifying via `m_last_operator` and do not need @call frames.
