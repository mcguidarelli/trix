...
`infix_primary` before the precedence-climbing loop runs, so they have no
numeric precedence value -- they always bind tightest.  `**` is listed at
...
- No new member variables or config options
