...
- **Nestable.** Up to 64 levels by default (configurable to 255). Inner
  restore rolls back to the inner checkpoint; outer state is preserved.
...
| -------------- | ------- | ----- | --------------------- |
| `m_save_count` | 64      | 4-255 | Maximum nesting depth |
...
> at scan time, since each local string cannot be stored into the global array.)
...
- **Entry writes (SetEntry, 12 bytes):** When `set-add` modifies an existing
  entry, the entire 12-byte SetEntry (key Object + chain link) is saved.
  The save-level check (`entry->m_key.save_level()` vs current save level)
  is identical to `Dict::put`.
- **Structural changes (SetEntryNext, 4 bytes):** When a new entry is
  inserted or `set-remove` relinks a chain, the predecessor's `m_next`
  field is journaled.
...
Each Name's binding cache carries a `m_binding_level`. On restore, any
binding with a level higher than the restored level is cleared. This
prevents stale cache entries from returning values in rolled-back
dictionaries.
...
| --- | --- | --- |
| Array element `put` (first write) | 20 bytes (entry + 8-byte Object) | Only the first write per slot per level |
| Array element `put` (repeat) | 0 bytes | Deduplication: same slot already saved |
| Dict `def` (existing key) | 32 bytes (entry + 20-byte DictEntry) | First write to this entry per level |
| Dict `def` (new key) | 32 + variable | DictEntry + DictHeader (once per dict per level) |
| Set `set-add` (existing key) | 24 bytes (entry + 12-byte SetEntry) | First write to this entry per level |
| Set `set-add` (new key) | 24 + variable | SetEntry + DictHeader (once per set per level) |
...
```
save /sv exch def
/x 1 def   % journal: save original x (32 bytes)
...
Save/restore supports up to 64 levels of nesting (configurable to 255). Each
...
- **restore:** Invalidates any binding with `m_binding_level` above the
  restored level. This prevents stale lookups to names in rolled-back
  dictionaries.
...
A string element is 1 byte. A journal entry is 12+ bytes (header + 1 byte
of data). Journaling every `put` on a string would consume 13x the memory
being protected. For a 1000-byte string being filled one byte at a time,
the journal would use 13,000 bytes to protect 1,000 bytes of data.

Array elements do not have this problem because each element is an 8-byte
Object. A journal entry (12 + 8 = 20 bytes) for an 8-byte element is a
2.5x overhead -- proportionate and acceptable.
