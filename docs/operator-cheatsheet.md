...
The everyday subset.  ~312 of the 828 user-facing operators in Trix,
...
| `copy`                | `a1..aN N -- a1..aN a1..aN` | Duplicate top N items                   |
| `roll`                | `aN..a1 n j -- a(j)..`      | Cyclic rotate                           |
| `index`               | `aN..a0 i -- aN..a0 a(i)`   | Copy by depth                           |
| `count`               | `-- n`                      | Items above mark                        |
| `clear`               | `... --`                    | Drop all                                |
...
| `single-bit?`                       | `int -- bool` -- power-of-two test  |
...
| `range range-from`                | Generate `[start..stop]`                               |
...
| `def store local-def`       | Bind in dict-stack          |
...
| `status` `query-status` | `-- dict` -- VM / runtime status info      |
...
| Op                                             | Effect                                              |
| ---------------------------------------------- | --------------------------------------------------- |
| `now`                                          | `-- ulong-instant` (ms since 1970 UTC)              |
...
| `status`                                       | `-- dict` -- runtime status dict                    |
...
| `actor-status` | `coroutine -- /running\|/suspended\|/dead\|...` |
...
| Op                                                      | Effect                              |
| ------------------------------------------------------- | ----------------------------------- |
| `supervisor-spec`                                       | Build supervisor spec dict          |
...
| `supervisor-get-child`                                  | `sup name -- spec`                  |
...
| `find-all`    | `template proc -- arr-of-results`    |
| `aggregate`   | `init step template proc -- result`  |
...
| Op | Effect |
| --- | --- |
| `coroutine-launch` | `proc -- coroutine` -- spawn new coroutine |
| `coroutine-suspend` | `val --` -- yield with value (resumer receives it) |
| `coroutine-resume` | `coroutine val -- val'` |
...
| Op            | Effect                                                  |
| ------------- | ------------------------------------------------------- |
| `match`       | `val pattern -- bindings-dict-or-null`                  |
...
| `handle-effect` | `proc handlers --` -- install effect handlers                   |
| `perform`       | `effect-name args -- result` -- raise an effect                 |
...
| Op | Effect |
| --- | --- |
| `now` | `-- ulong` -- ms since 1970 UTC |
| `clock` | `-- ulong` -- monotonic microseconds |
| `instant-to-date` `instant-to-date-local` | `ulong -- arr` -- decompose to date fields |
