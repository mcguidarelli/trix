...
indexing. See `vm_heap.inl` for the implementation.
...
`Dict::name_lookup()` handles both cases in its inner loop: when the entry
...
The first time a name is looked up (cache miss), `Dict::name_lookup()` walks
...
%   2. Dict::name_lookup: walk dict stack top to bottom
...
name_search(key):
    ...
    3. DICTIONARY STACK: Dict::name_lookup(trx, key)
...
On a cache miss for a normal name, `Dict::name_lookup()` searches the
...
The outer loop iterates over 2-4 dicts in typical usage (systemdict +
userdict + 0-2 frame dicts). Each dict lookup is O(m) where m is the
