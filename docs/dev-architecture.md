...
Immediately after the closing brace a `static_assert` runs
`verify_dispatch_tables()` to prove the operator tables are complete at compile
time.
...
bottom of the stack; `userdict` sits above it; `begin` / `use` push further
scopes.  See [Name Lookup](name-lookup.md) and [Collections](collections.md).
