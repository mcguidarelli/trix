...
- `limit-check` -- repeat count or total format size exceeds 65535
...
the input.  The narrow targets `b` (int8) and `h` (int16) range-check against the
target width and raise `range-check` on overflow; `i` (int32) and `l` (int64) are
written by a truncating cast (no range-check — an out-of-range value wraps).
...
- `length_t repeat` -- repeat count (1 if not specified; for `s`, `repeat` is reset to 1)
- `std::endian endian` -- current endianness state
- `length_t spec_size` -- bytes per element for this specifier; for `s`, this is the string byte length
...
Five helper functions extract values from Trix Objects for packing:
...
- `pack_extract_int128(trx, arg, spec)` -- returns `int128_t` for the `q` specifier; widens narrower integers
- `pack_extract_uint128(trx, arg, spec)` -- returns `uint128_t` for the `Q` specifier; rejects negatives
...
| `limit-check` | Repeat count or total format size > 65535 | `mark 1 (70000B) pack` |
