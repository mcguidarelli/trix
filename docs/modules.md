...
/math-utils {
    /square { |x| x x mul } def
    /cube { |x| x x mul x mul } def
    /my-abs { |x| x 0 lt { x neg } { x } if-else } def
    /clampf { |x lo hi| x lo max hi min } def
...
/math-utils /square /my-abs 2 import
-5 my-abs square     % => 25
```
...
block-beta
    ...
    block:row1:3
        columns 3
        a1["Modules registry"] a2["~804"] a3["16 header + 148 buckets + 640 pool (32 * 20)"]
    end
    block:row2:3
        columns 3
        b1["Module dict (empty)"] b2["~804"] b3["Cap-32 pool allocated eagerly at create"]
    end
    block:row3:3
        columns 3
        c1["Module dict (1..32 entries)"] c2["~804"] c3["Entries 1..32 add ZERO incremental cost"]
    end
    block:row4:3
        columns 3
...
A module with 10 entries costs approximately ~804 bytes of VM heap (the 32-entry
pool is pre-allocated, so a 10-entry module fits without growth; only the 33rd
entry triggers expansion, growing the pool by 32 -> +640 bytes).  The modules
registry itself costs ~804 bytes (allocated once at startup).
...
5. **Fixed initial capacity.** Module dicts start at capacity 32.  A module with
   up to 32 entries fits without expansion; the 33rd entry triggers dynamic
   expansion (the pool doubles to 64).
