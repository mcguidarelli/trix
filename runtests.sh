...
# --- Step 1f: DWARF reader (lib/dwarf.trx against a compiled fixture) ---
echo ""
echo "===================================="
echo "  Running tests/run_dwarf_tests.sh (DWARF reader)"
echo "===================================="

set +e
DWARF_OUTPUT=$(tests/run_dwarf_tests.sh 2>&1)
DWARF_EXIT=$?
set -e
echo "$DWARF_OUTPUT"

DWARF_RESULTS=$(grep -m1 "DWARF tests:" <<< "$DWARF_OUTPUT" | sed 's/.*DWARF tests: //' || echo "?? passed, ?? failed")
if [ "$DWARF_EXIT" -ne 0 ]; then
    FAIL=$((FAIL + 1))
    ERRORS="${ERRORS}  FAIL: DWARF reader tests failed (exit ${DWARF_EXIT})\n"
fi

DWARF_RTOK_FAILS=$(grep -c "^FAILED:" <<< "$DWARF_OUTPUT" || true)
if [ "$DWARF_RTOK_FAILS" -gt 0 ]; then
    FAIL=$((FAIL + 1))
    ERRORS="${ERRORS}  FAIL: ${DWARF_RTOK_FAILS} rt-ok assertion(s) failed inside DWARF tests:\n"
    while IFS= read -r line; do
        ERRORS="${ERRORS}    ${line}\n"
    done < <(grep "^FAILED:" <<< "$DWARF_OUTPUT")
fi
