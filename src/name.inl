...
//     [next (vm_offset_t)] [hash (hash_t)] [length (length_t)]
//     [data (vm_t[length])]
...
//
// BINDING CACHE
//   The name->value binding cache is per-coroutine, not per-Name.  Each
//   CoroutineContext owns a small chained hash table keyed by Name offset
//   (see binding_table.inl); the running coroutine's table is consulted
//   first in name_search(), and a miss falls back to the dict-stack walk.
//   This eliminates the old Name::m_binding race where two actors that
//   def'd the same name clobbered each other's binding across yields.
//
// NAME CATEGORIES
...
//   1. Binding cache hit: O(1) if m_binding is valid at current save level.
...
// faster scans at 32771 buckets (release-audit/phase5/perf-analysis.md).
