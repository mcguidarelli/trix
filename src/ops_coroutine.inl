...
//
//   CoroutineContext (~184 bytes, static_assert <= 192):
//   +-------------------------------------------+
...
//   +----------+----------+-------+-----------+
//   | operand  | execution| error | dictionary|
//   | [128]    | [128]    | [8]   | [8]       |
//   +----------+----------+-------+-----------+
//   Each section is DefaultCoroutine*Depth Objects.
//
...
//
// SCHEDULING
//   The current scheduler is a simple round-robin walk of a circular
//   singly-linked list containing all live coroutines.  On each schedule
//   call, it finds the next Ready or timer-expired Sleeping coroutine.
//
//   If no coroutine is ready and all are sleeping, the scheduler sleeps
//   the real OS thread (nanosleep) until the earliest wake time.
//
...
//
//   Note: the scheduler is O(n) per schedule call, where n is the total
//   coroutine count.  See docs/plan-scheduler.md for the planned O(1)
//   redesign using separate ready queue and timer list.
//
// LIFECYCLE
...
//   coroutine_remove_dead().  Their stack blocks and context structs are
//   returned to free lists for reuse by future coroutine launches.
//
// COROUTINE LAUNCH
...
void gc_walk_coroutine_context(vm_offset_t payload_offset) {
    ...
    // as a use-after-free / double-free (release-audit/b7-gc-stress-findings.md
    // Finding 4).  A DEAD coroutine also has m_stacks_offset == nulloffset, but its
