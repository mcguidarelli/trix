...
//         resident storage.
//       Special: SourceLoc reuses save_level byte as stream_id and
//         length field as source line (internal only, never user-visible).
//
...
//   Bit 5 (F): Special flag -- type-specific (e.g., DynamicFlag for dicts,
//     IsMemory for streams, packed-type compression flag).
...
//   | 0   1   2   3 | 4   5   6   7 |
//   +---+---+---+---+---+---+---+---+
//   | dead_sentinel | 64-bit value  |
//   |  (or free-    | (Long, ULong, |
//   |   list next)  |  Double, Addr)|
//   +---+---+---+---+---+---+---+---+
//   Live: dead_sentinel == 0, value holds the 64-bit datum.
//   Free: dead_sentinel == sm_dead_sentinel, value holds free-list next offset.
//
...
// 32 bits (Byte, Integer, UInteger, Real, Boolean, Operator) are stored inline.
...
public:
    ...
    //
    //  Boolean (T=01001):
    //    F carries the Boolean value itself (0 = false, 1 = true).
    //    This is the primary data slot, not a flag.
    //
    //  Address (T=00110):
    ...
    make_empty_string(Trix *trx, length_t length, attrib_t attrib = LiteralAttrib, access_t access = ReadWriteAccess) {
        auto [ptr, offset] = trx->vm_alloc_dispatch_n<vm_t>(length + 1, Trix::ChunkKind::String);
