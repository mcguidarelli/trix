...
//   for space efficiency.  Trix extends this with 32 PackedType encodings
...
//   regular Object.
//
//   pack/unpack convert between Array and Packed representations.
//   Packed arrays are read-only and sequential-access only (no O(1) indexing).
//   They are the default representation for procedure bodies created by { }.
//
// BINARY PACK/UNPACK
//   binary-pack encodes Trix values into a byte string using a format
//   descriptor (similar to Python's struct.pack).
//   binary-unpack decodes a byte string back into Trix values.
//   Supports: byte, integer (big/little endian), real, double, string
//   (fixed and length-prefixed), and padding.
//
// PEEK/POKE
...
//
// ADDRESS ARITHMETIC
//   address-add, address-sub, address-diff provide pointer arithmetic
//   within the VM heap.  Addresses are vm_offset_t values (uint32_t
//   byte offsets from m_vm_base).
//
// PACKED ARRAY ENCODING (PackedType enum + encoding logic in object.inl)
//
//   Each element is encoded as:
//     [header] [save_level?] [length?] [value?]
//
...
//   ENCODING BY TYPE:
//
//   Type                       Header  SL  Len   Value   Notes
//   -------------------------  ------  --  ----  ------  ----------------------------
...
//   Long (5)                   1       1   -     1-4     vm_offset_t to ExtValue
//   ULong (6)                  1       1   -     1-4     vm_offset_t to ExtValue
//   Address (7)                1       1   -     1-4     vm_offset_t to ExtValue
//   Real (8)                   1       -   -     1-4     32-bit float bit pattern
//   Double (9)                 1       1   -     1-4     vm_offset_t to ExtValue
...
//   Save (16)                  1       -   -     1-4     vm_offset_t (save barrier)
//   ShortLengthArray (17)      1       -   1     1-4     vm_offset_t to Object[]
//   LongLengthArray (18)       1       -   2     1-4     vm_offset_t to Object[]
//   ROShortLengthArray (19)    1       -   1     1-4     read-only
//   ROLongLengthArray (20)     1       -   2     1-4     read-only
//   ROShortLengthPacked (21)   1       -   1     1-4     vm_offset_t to packed data
//   ROLongLengthPacked (22)    1       -   2     1-4     vm_offset_t to packed data
//   ShortLengthString (23)     1       -   1     1-4     vm_offset_t to byte[]
//   LongLengthString (24)      1       -   2     1-4     vm_offset_t to byte[]
//   ROShortLengthString (25)   1       -   1     1-4     read-only
//   ROLongLengthString (26)    1       -   2     1-4     read-only
//   ShortLengthStream (27)     1       -   1     1-4     vm_offset_t to Stream
//   LongLengthStream (28)      1       -   2     1-4     vm_offset_t to Stream
//   ROShortLengthStream (29)   1       -   1     1-4     read-only
//   ROLongLengthStream (30)    1       -   2     1-4     read-only
//   Dict (31)                  1       -   -     1-4     vm_offset_t to Dict
//                                                        (Set shares via X bit)
//
//   TOTAL SIZE per element: header(1) + save_level(0|1) + length(0|1|2) + value(0-4)
//   Minimum: 1 byte (CommonOp, Simple)
//   Maximum: 8 bytes (Long/ULong/Address/Double with 4-byte offset + save level)
//   Typical: 1-3 bytes (most operators and small integers)
//
//   NOT PACKABLE: Thunk, Coroutine, PipeBuffer, Cell, SourceLoc
...
// (object-to-binary-token, make-binary-token-string), and bit-position
// template for find-first/find-last operations.
//
...
static void peek_op(Trix *trx) {
    if (trx->m_sandbox) {
        ...
        if (!is_name) {
            ...
            if ((TypeToVerify(type) & VerifyNumbers) == 0) {
...
static void inflate_stream_op(Trix *trx) {
    ...
    if (!in_stream->is_readable(in_sid)) {
        ...
        if (!out_stream->is_writable(out_sid)) {
