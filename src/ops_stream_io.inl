...
//
//   PostScript file I/O: file (open-file), closefile (close-stream),
//   read, write, readstring (read-string), writestring (write-string),
//   bytesavailable (bytes-available), flush (flush-stream),
//   setfileposition (set-file-position), fileposition (file-position),
...
//
// FILE OPERATIONS
//   open-file    -- open a file by name with mode (r/w/a/r+/w+/a+)
...
//
// SEEK OPERATIONS
//   set-file-position -- seek to absolute position
//   file-position     -- query current position
//
// WITH-STREAM PATTERN
//   `filename mode { |stream| ... } with-stream` opens a file, binds
//   the stream to a local variable, executes the proc, and guarantees
