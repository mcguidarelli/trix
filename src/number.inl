...
// Long, ULong, Address, Real, Double).  Based on:
//
...
//   type suffixes (b, i, u, l, ul, a, r, d), C-style prefixes (0x, 0o, 0b),
...
//                ul (ulong), a (address), r (real), d (double)
//
// TYPE INFERENCE
//   Without an explicit suffix, the parser infers the narrowest type:
//   - Integer for values in [-2^31, 2^31-1]
//   - UInteger for values in [2^31, 2^32-1] (unsigned context)
//   - Real for decimal values with a fractional part or exponent
//   With a suffix, the parser coerces to the specified type (with range
//   checking and overflow detection).
//
// OVERFLOW HANDLING
//   Integer overflow during parsing is detected and reported as a
//   RangeCheck error.  The parser uses unsigned 64-bit accumulation
//   with explicit overflow checks before each multiply/add.
//
