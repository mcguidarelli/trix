...
//   copy, mark, counttomark (count-to-mark).  These are the fundamental data-movement
//   primitives in a stack-based language -- all computation depends on
//   arranging operands correctly on the stack.
//
//   Forth: swap (exch), rot, over, nip, tuck, pick (index), depth (count).
...
//
// COPY SEMANTICS
//   `copy` is polymorphic by operand type:
//   - Integer: copy N stack elements (PostScript-style stack copy)
...
//   `counttomark` counts elements above the nearest mark.
...
//   copy         int | src dst --      Copy N elements or container contents
//   mark         -- mark               Push mark sentinel
//   counttomark  mark any* -- mark any* int  Count above mark
//   rot          a b c -- b c a        Rotate top three
//   over         a b -- a b a          Copy second element
//   nip          a b -- b              Discard second element
//   tuck         a b -- b a b          Copy top below second
//   pick         any_n ... any_0 n -- any_n ... any_0 any_n  (alias for index)
//   depth        -- int                (alias for count)
