...
//
// STRING HANDLING
//   All strings are passed as NUL-terminated C strings (via c_str()) to
//   the POSIX regex API.  Trix strings may not be NUL-terminated
//   internally, so c_str() copies to a NUL-terminated VM buffer when
//   needed.  Results (match groups, replacements) are constructed as new
//   Trix strings on the VM heap.
//
// --- Operators ---
//
