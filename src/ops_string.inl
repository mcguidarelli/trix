...
//   and a length in the header.  Strings are NOT null-terminated internally
//   (c_str() adds a terminator when needed for C API calls).  Strings can
...
//   trim_impl() and pad_string_impl() are shared helpers for trim/pad
