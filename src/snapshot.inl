...
//   - The SnapShotHeader: ~90 root pointers and scalar fields that
...
//
// SNAPSHOTHEADER
//   A large struct (~90 fields) that captures every Trix member variable
...
struct RestoredMemStream {
    vm_offset_t offset;
    vm_t *buf;
    size_t remaining;  // written/read as sizeof(size_t) bytes -- 64-bit builds only
};
// The file format stores 'remaining' with native sizeof(size_t) width.  A future
// 32-bit port would require a format bump; document the assumption explicitly.
