...
// operators.  Each shim is a small function that handles one specific type
// combination (e.g., Integer + Real, ULong * Byte).  The dispatch table in
// dispatch.inl selects the correct shim based on the two operand types.
//
// Based on:
//
...
//
//   C++ template specialization: each shim is a static function template
//   instantiation that reads the operands, performs the type-specific
//   operation, and writes the result.
//
// --- Core concepts for maintainers ---
//
// SHIM NAMING CONVENTION
//   shim_O{InputTypes}__R{ResultType}
//   Example: shim_OUnsignedIntegers__RInteger handles Byte/UInteger/ULong
//   inputs producing an Integer result.
//
// DISPATCH FLOW
//   1. Binary operator (e.g., add_op) is called.
//   2. It calls the shim dispatch function with both operand types.
//   3. The dispatch function indexes a 2D table by (type1, type2).
//   4. The table entry is a function pointer to the correct shim.
//   5. The shim reads operands, computes, writes result, manages ExtValues.
//
// WHY SHIMS (not templates or virtual dispatch)
//   The shim approach generates a flat function-pointer table with O(1)
//   dispatch and no virtual call overhead.  Each shim is small (5-15 lines)
//   and fully inlined by the compiler.  The total number of shims is
//   bounded (each operator has ~20-30 valid type combinations).
//
...
//
// DO NOT REFACTOR
//   The shim functions are intentionally NOT consolidated into templates
//   or generic dispatch.  Each shim's explicit type handling is a
