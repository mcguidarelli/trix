...
//
//   PostScript array operations: array, aload (explode), astore (implode),
//   get, put, length, getinterval (sub-array), putinterval (put-interval),
//   copy, for-all.  Trix names in parentheses where they differ.
//
//   Functional collection operations (map/filter/reduce are in
//   ops_higher.inl and ops_array_iteration.inl): sort, reverse, flatten,
...
//
// SORTING
//   sort uses std::sort with Object::compare() for default ordering.
//   sort-by uses a key-extraction proc with the trampoline pattern
//   (key is computed via exec stack, then comparison proceeds).
//   Both allocate a new array (sort is not in-place on the original).
//
...
//   unique similarly uses a dict to track seen elements.
//
