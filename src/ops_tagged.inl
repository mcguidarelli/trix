...
//   Error results:    `{ ... } try` wraps caught errors as `/error` tagged
//
// PATTERN MATCHING
...
//   try           proc -- result                Execute, wrap error as /error tagged
//   try-catch     proc handler -- result        Execute, call handler on error
//
// Note: `try` and `try-catch` are in ops_flow.inl but produce tagged
// results (value /ok tag or error-dict /error tag).
//
