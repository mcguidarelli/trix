...
//   Each byte value (0-255) has a precomputed attribute in the chattr[]
//   table (scanner_tables.inl).  Attributes classify characters as:
...
//
//   Strings:
//     (text)       standard string with escape sequences:
//                    \n \t \r \\ \( \) \xHH \0-3 digits (octal)
...
//
//   String suffixes (apply to (...) and <(...)> strings):
...
//     Proc suffixes: #= (use =proc), #e (early bind), #l (late bind),
//       #a (array not packed), #p (force packed), #r (read-only),
//       #w (read-write), #pw (packed + read-write).
