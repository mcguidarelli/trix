...
// (`vm_alloc_global_ptr` in vm_heap.inl) and three internal callers
...
// principles against:
//
//     * a single contiguous zone bounded by `m_vm_global` and
//       `m_vm_limit` (top-of-heap; grows DOWNWARD as new allocations
//       are made);
...
// frees are C++-internal only (ExtValue::free / WideValue::free, and the
// reclaim of an overwritten global Dict / ExtValue / WideValue slot).
//
// All allocator logic lives here and only here; vm_heap.inl, name.inl,
// dict.inl, object.inl, and scanner.inl reach it through
// vm_alloc_global_ptr but hold no allocator state of their own (modulo the
// C++-internal free callsites in object.inl that wire ExtValue::free /
// WideValue::free through to gvm_free).
//
//===--- Block layout ------------------------------------------------------//
//
...
//
// gvm_heap.inl never reads or writes m_vm_ptr / m_vm_temp_ptr
// directly -- it only consumes m_vm_global / m_vm_limit for walks,
// and vm_alloc_global_ptr for forward growth.  This keeps
