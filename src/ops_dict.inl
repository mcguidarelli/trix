...
// Exec-stack frame (5 slots).  Layout while @dict-for-all is on the stack
// (as seen by dict_forall_clear / dict_forall_update, ptr -> @dict-for-all):
//
//   ptr[0]  @dict-for-all
//   ptr[-1] offset       (vm_offset_t of next Entry, or nulloffset when done)
//   ptr[-2] bucket_idx   (integer_t; -1 on first call, bucket index thereafter)
//   ptr[-3] dict
//   ptr[-4] proc
//
// Layout inside at_dict_forall_op (interpreter has already popped @dict-for-all,
// so m_exec_ptr has been decremented; m_exec_ptr -> offset):
//
//   enter (m_exec_ptr ->)  offset      exit (m_exec_ptr ->)  proc
//                          bucket_idx                        @dict-for-all
//                          dict                              offset
//                          proc                              bucket_idx
//                                                            dict
//                                                            proc
...
static void def_persist_op(Trix *trx) {
    ...
    if (!dict->has_write_access()) {
        ...
        if (Save::is_active(trx)) {
            ...
            // sl=0: silent degrade to def's binding semantics.
            dict->put(trx, *key_ptr, *val_ptr, Dict::BindingMode::Bind);
        }
        trx->m_op_ptr -= 2;
    }
...
static void bind_into_dict_op(Trix *trx) {
    ...
    if (keys_length == 0) {
        ...
        for (length_t i = 0; i < keys_length; ++i) {
            dict->put(trx, keys_base[i].make_clone(trx), values_base[i], Dict::BindingMode::Bind);
        }
