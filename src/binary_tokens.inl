...
// tokens provide a compact bytecode representation for Trix programs,
// used by snap-shot/thaw serialization and packed array storage.  Based on:
//
...
//   in the byte stream (0-8 bytes depending on the type).
//
//   The BinaryToken enum (enums.inl) maps opcode bytes to token types:
//     Byte, Integer8/16/32, UInteger8/16/32, Long, ULong, Real, Double,
//     Address, Boolean, Null, Mark, SystemName8/16, UserName16,
//     String8/16/32, Array8/16, Packed8/16, Operator8/16, etc.
//
// DECODER FUNCTIONS
//   Each bt_* function decodes one token type:
//     1. Read the payload bytes from the stream (big-endian).
//     2. Construct the corresponding Object.
//     3. Return (Lexeme, Object) pair for the scanner.
//
// ENDIANNESS
//   Multi-byte payloads are stored in big-endian (network byte order)
//   for portability.  Decoders use explicit byte assembly rather than
//   relying on host endianness.
//
// ERROR HANDLING
//   If the stream ends mid-token (incomplete payload), the decoder
//   returns (EndOfStream, error-string).  Invalid opcodes raise
//   InternalError.
//
