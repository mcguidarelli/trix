...
//   @supervisor-init     Move state_offset op->exec, enter recv loop
//                        (children are already spawned synchronously in
//                        supervisor_op before the coroutine ever runs)
//   @supervisor-loop     Main supervisor message loop (handle /down)
//
                                    ...
                                    if (!child_specs[i].is_dict()) {
                                        ...
                                        if ((cid == nullptr) || !cid->is_name()) {
                                            ...
                                            if (cstart == nullptr) {
                                                ...
                                                if (cstart->is_literal() || !cstart->is_callable()) {
                                                    ...
                                                    if ((crestart == nullptr) || !crestart->is_name()) {
                                                        trx->error(Error::TypeCheck,
                                                                   "supervisor: child {}: /restart must be a name",
                                                                   i);
                                                    } else {
...
static void supervisor_count_children_op(Trix *trx) {
    ...
    if (target->m_status == CoroutineContext::Dead) {
        ...
        if (target->m_return_value.is_null() || !target->m_return_value.is_integer()) {
...
static void supervisor_spec_op(Trix *trx) {
    ...
    if (target->m_status == CoroutineContext::Dead) {
        ...
        if (target->m_return_value.is_null() || !target->m_return_value.is_integer()) {
            ...
            dict->put(trx,
                      trx->wellknown_name(WellKnownName::IKeyIntensity),
                      Object::make_integer(static_cast<integer_t>(intensity)));
            dict->put(trx, trx->wellknown_name(WellKnownName::IKeyPeriod), Object::make_integer(static_cast<integer_t>(period)));
            dict->put(trx,
                      trx->wellknown_name(WellKnownName::IKeyChildCount),
                      Object::make_integer(static_cast<integer_t>(child_count)));
            dict->put(trx,
