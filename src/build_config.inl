...
// plus `#include` / `#pragma`.  See STYLE.md ("Language Restrictions") for
...
// vm-heap-snapshot / vm-heap-diff ops + heap-track / alloc status keys.
...
// [[unlikely]]) plus the DebugState struct (Mode + a few small ints/
// offsets + a callback Object) and the m_debug_proc_lines per-op source-
// line map (the sole std::unordered_map/std::vector heap-STL carve-out)
