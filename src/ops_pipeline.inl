...
//   The header contains read/write indices, occupancy, capacity, closed
//   flag, error code, and producer/consumer/blocked coroutine offsets.
//
// --- Operators ---
//
                    ...
                    if (!sources_val->is_array()) {
                        ...
                        if (sources_len > MaxPipeStages) {
                            trx->error(Error::LimitCheck,
                                       "pipeline: too many merge/zip sources ({}, max {})",
                                       sources_len,
                                       MaxPipeStages);
                        } else {
                            ...
                            if (result.coroutine_count >= MaxPipeCoroutines) {
                                ...
                                result.coroutines[result.coroutine_count++] = pipe_create_merge_coroutine(
                                        trx, first_buf, sub_sink_bufs, static_cast<int32_t>(sources_len));
                            } else {
