...
//
// Unary operators (-, +, !, ~) bind LOOSER than `**`.  `-2**2` parses
// as -(2**2) = -4, matching Python / MATLAB / Fortran / Ruby.  See
// infix_primary for the implementation: unary `-` recurses into
// infix_expr at min_prec = 13 so `**` is bound before `neg` is emitted.
//
