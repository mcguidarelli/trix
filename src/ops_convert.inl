...
//     non-numeric types.  Uses cast_to_type<T> template for type-safe
...
//   Recursive structural comparison of containers (arrays, records, sets,
//   dicts, tagged values).  Depth-limited by MaxRecursionDepth to guard
//   against cycles.  Leaf values use Object::equal() (shallow).
//
// ATTRIBUTE MODIFICATION
//   make-executable / make-literal:  set/clear the Execute bit
//   make-readonly / make-readwrite:  set/clear the Access bit
//   These produce new Objects with modified attributes; they do not mutate.
//
// TYPE PREDICATES
//   is-type: name -- bool (checks if an object is of the named type)
//   type: any -- name (returns the type name of an object)
//   All 29 types have a corresponding type name in systemdict.
//
...
// type predicates (is-number, is-type), equality comparison (eq, ne,
// deep-eq, deep-ne), type-name / error-name lookup, and homogeneous type check.
//
        ...
        if (a.is_sequence() && b.is_sequence()) {
            ...
            if ((a_type == b.type()) && a.is_deepeq()) {
                if (a_type == Object::Type::Curry) {
                    ...
                    return (deep_equal(
                                    trx, a_storage[Object::ThunkStorageState], b_storage[Object::ThunkStorageState], depth + 1) &&
                            deep_equal(trx, a_storage[Object::ThunkStorageProc], b_storage[Object::ThunkStorageProc], depth + 1) &&
                            deep_equal(
                                    trx, a_storage[Object::ThunkStorageResult], b_storage[Object::ThunkStorageResult], depth + 1));
...
[[nodiscard]] std::pair<bool, Error> is_error_name(const Object *val_ptr) {
    if (val_ptr->is_name()) {
        for (int i = 0; i < ErrorCount; ++i) {
            if (m_errorname_offsets[i] == val_ptr->m_name) {
