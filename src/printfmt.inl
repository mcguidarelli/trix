...
//   O (object form), ? (boolean as "true"/"false"), and I (chrono
//   instant; ULong ms since 1970 UTC formatted via strftime template).
//
...
//   e/E/f/F/g/G; strings support s; booleans support ?; ULong adds I
//   (chrono instant via strftime template); all types support T (type
//   name) and O (object form).  The "#" alt-flag adds 0x/0X/0b/0o
//   prefixes to integer output.
//
// ARGUMENT ACCESS
//   Arguments are accessed by positional index (implicit auto-increment
//   or explicit {N}).  Out-of-range indices raise Error::InvalidFormatString.
            ...
            if (m_type == 'O') {
                ...
                if (ctx->m_mailbox != nulloffset) {
                    ...
                    if (debug_name.empty()) {
                        ...
                        auto [ptr, _] = std::format_to_n(buffer,
                                                         sizeof(buffer),
                                                         "--{}:{}:{}:{}/{}--",
                                                         sv,
                                                         debug_name,
    ...
    void print_cell(const Object *val_ptr) {
        ...
        if ((m_type != 's') && (m_type != 'O')) {
            ...
            if (m_type == 'O') {
                ...
                // Extract debug name if present
                auto debug_name =
                        (header->m_debug_name != nulloffset) ? m_trx->offset_to_ptr<Name>(header->m_debug_name)->sv() : ""sv;
