...
//   produce separate schemas).  Equality checks compare field names, not
//   schema offsets.
//
// RECORD CREATION
//   Two paths:
...
//
// RECORD EQUALITY
//   Two records are equal if they have the same number of fields, the same
//   field names (in the same order), and equal field values (shallow).
//   Use deep-eq for recursive structural comparison of nested records.
//
// UNIFICATION
...
//   record-for-all  record proc --          Iterate name/value pairs
//
...
//   [field_count (uint32_t)] [Name[0]] [Name[1]] ... [Name[N-1]]
// Record instance: stored in VM heap, one per record value.
//   [schema_offset (vm_offset_t)] [Object[0]] [Object[1]] ... [Object[N-1]]
// Both use uint32_t header fields so Object[] starts at 4-byte alignment.
struct RecordSchema {
