...
//   Only set-add and set-remove mutate in place (on read-write sets).
//
// SET CREATION
//   set             -- create a set from mark-delimited elements
//   set-from-array  -- create a set from an array
//   =set            -- push the temporary =set (cleared)
//   Duplicates are silently ignored during creation.
//
// ITERATION
//   set-for-all iterates over all elements (order is hash-dependent and
//   not guaranteed).  set-filter creates a new set from elements matching
//   a predicate.  set-members converts to an array.
//
// --- Operators ---
//
//   set                    mark any* -- set     Create from elements
...
//   set-symmetric-difference  set set -- set    Symmetric difference (new set)
//   set-subset?            set set -- bool      Subset test
//   set-disjoint?          set set -- bool      Disjoint test
//   set-members            set -- array         Convert to array
//   set-filter             set proc -- set      Filter by predicate (new set)
//   set-for-all            set proc --          Iterate elements
