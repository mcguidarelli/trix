...
//     1. Flush all dirty stream buffers.
//     2. Write the SnapShotHeader (root pointers, member variables).
//     3. Write the VM heap (m_vm_base to m_vm_ptr) as raw bytes.
//     4. For each open file stream: write filename + file position.
...
//   position.  Memory streams (from invoke API) are tracked via
//   RestoredMemStream (snapshot.inl) and require the host to provide
//   the buffer during thaw.
//
// --- Operators ---
//
                    ...
                    if (h.vm_used > vm_capacity) {
                        ...
                        if ((h.vm_global_used > 0) && (h.vm_capacity != vm_capacity)) {
                            ...
                            if (h.vm_global_used > (vm_capacity - h.vm_used)) {
                                trx->diag_println("thaw: local + global ({} + {}) exceeds VM capacity {}",
                                                  h.vm_used,
                                                  h.vm_global_used,
                                                  vm_capacity);
                                return false;
                                ...
                                if (h.memory_stream_count > static_cast<uint32_t>(MaxStreamCount)) {
                                    trx->diag_println("thaw: memory stream count {} exceeds maximum {}",
                                    ...
                                    for (uint32_t i = 0; i < h.memory_stream_count; ++i) {
                                        ...
                                        if (!read_all(fd, &off, sizeof(off)) || !read_all(fd, &rem, sizeof(rem))) {
                                            ...
                                            if (buf == nullptr) {
                                                trx->diag_println(
                                                        "thaw: malloc failed for memory stream block {} ({} bytes)", i, rem);
                                                return false;
                                    ...
                                    if (!read_all(fd, trx->m_vm_base, h.vm_used)) {
                                        ...
                                        if (crc != saved_crc) {
                                            trx->diag_println(
                                                    "thaw: checksum mismatch (file={:#010x}, computed={:#010x})", saved_crc, crc);
                                            return false;
                                        } else {
                                            ...
                                            if ((h.memory_stream_count > 0) && (computed_mem_crc != h.memory_stream_crc)) {
                                                ...
                                                if ((h.user_file_stream_count > 0) &&
                                                    (computed_ufs_crc != h.user_file_stream_crc)) {
                                                    ...
                                                    // both count and name mismatches.
                                                    {
                                                        ...
                                                        crc32_t live_crc = 0;
                                                        if (trx->m_useroperators != nullptr) {
                                                            for (auto p = trx->m_useroperators; p->m_func != nullptr;
                                                                 ++p, ++live_count) {
                                                                auto sv = p->m_sv;
                                                                auto len = static_cast<uint16_t>(sv.size());
                                                    ...
                                                    // span.
                                                    apply_fixup_streams(trx,
                                                                        std::span<const RestoredMemStream>(restored_mem_streams,
                                                                                                           restored_count),
                                                                        {});
                        ...
                        if (ufs_section_start == -1) {
                            ...
                            for (uint32_t i = 0; i < h.user_file_stream_count; ++i) {
                                ...
                                if (fnlen >= sizeof(path_scratch)) {
                                    ...
                                    if ((fnlen > 0) && !read_all(fd, path_scratch, fnlen)) {
                                        trx->error(
                                                Error::InvalidImageFile, "thaw: user file stream block {} filename read failed", i);
                                    } else {
