...
//   1. Allocate VM heap (single malloc, default 256KB).
//   2. Initialize member variables to defaults.
//   3. Allocate the four stacks (operand, execution, error, dictionary)
//      from the VM heap.
//   4. Allocate eq-buffers (=string, =array, =proc, =dict, =set).
//   5. Allocate name hash table buckets.
//   6. Allocate error string buffer.
//   7. Call Dict::init() to create systemdict, userdict, errordict,
//      handlersdict, numbersdict -- populating ~700 operators.
//   8. Initialize streams (stdin, stdout, stderr, stdedit).
//   9. Initialize well-known name offset table (WellKnownName enum).
//  10. Push startup file/stream/image onto the execution stack.
//  11. Call interpreter() -- the main loop.
//
// DESTRUCTOR
//   Sends ExitIRQ to stop the interpreter, joins the interpreter thread
//   (if threaded), frees the VM heap, and cleans up readline state.
//
// STARTUP MODES
        ...
        if (::pipe2(m_pipe_fds, O_NONBLOCK) == -1) {
            ...
            if (mode == StartupMode::ImageFile) {
                ...
                *barrier_ptr = nulloffset;
                ...
                // {{ }}#=
                {
                    m_eqset_maxlength = eqdict_maxlength;
