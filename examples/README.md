...
### [amazing.trx](amazing.trx) — 11-algo maze zoo + 5-grid topology + PNG output + bitmap font

A self-contained maze generator (~6,900 lines), a homage to Steve Capps' "Amazing" on the 128K Mac.  Ships **eleven** maze algorithms (recursive-backtracker, Kruskal, Wilson, Eller, binary-tree, sidewinder, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree, and recursive-division -- the one wall-adding generator) across **five** grid topologies -- square (the full zoo), plus hex, theta polar, triangle, and upsilon octagon.  Seven of the eleven algorithms (backtracker, Kruskal, Wilson, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree) run on **every** grid through a shared topology descriptor; Eller/binary-tree/sidewinder/division stay square-only -- 35 portable combos.  Renders BFS distance-field heatmaps in ten colormaps with `--solve` / `--braid` / `--weave` overlays, and a `--metrics` quality report on any grid.  **Masking** carves the maze into a shape -- a word (`--mask-text 'Amazing!'`), the real Trix logo cut out of a maze (`--mask logo`), any SVG (`--mask-file`), or an analytic disc/ring/frame -- with `--mask-invert` to punch the shape out of a full maze instead; disconnected shapes (letters) become one perfect maze per connected piece.  Text renders through a selectable `--font`: a built-in 5×7 block font, hi-res **Roboto** bitmap atlases (Apache-2.0), and **Hershey** stroke fonts (public domain) rendered in pure Trix -- generated from real faces by host tools (`tools/gen_mask_font.py` / `gen_hershey_font.py`), only the derived glyph data committed (see [`mask-fonts/`](mask-fonts/)).  The logo and any other SVG are rasterised to a 1-bit mask by `tools/gen_mask_svg.py` (only the logo's derived mask committed, see [`mask-shapes/`](mask-shapes/)).  The maze line and background colors are themeable (`--wall-color` / `--bg-color`, hex) on every topology.  Output is a real PNG whose format is assembled in Trix (no libpng) over the engine's native `deflate` (zlib) and hand-rolled `crc32`/`adler32` ops.  Companion teaching + implementation guide: [amazing-manual.md](amazing-manual.md).  `--self-test` runs 194 assertions; [`gallery.sh`](gallery.sh) renders an 88-PNG showcase (including the full algorithm-by-topology matrix) into `examples/maze-gallery/` (gitignored).  Needs `--vm-size=8M`+ (more for `--stress`); see the manual for sizing.
