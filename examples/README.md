...
Six fields in a mutable dict: `(x, y)` Real-typed position, `(dx, dy)` facing-direction unit vector, `(px, py)` camera-plane vector perpendicular to dir with length `tan(fov/2)` (~0.66, the classic Wolfenstein 66-degree FOV).  This is the Lode Vandevenne representation; storing as a dict (not a Record) keeps per-frame updates as in-place puts.
...
- **Default** (`shading-mode = /none`): the side shift is the entire offset.  All walls render at full brightness regardless of distance; perspective alone communicates depth.  This is the Wolfenstein-3D aesthetic and gives the cleanest ASCII output.
...
## Bestagon prototypes and balance sims

Two subdirectories support the in-design Bestagon hex-strategy game
and are internal working artifacts rather than curated showcases:
[`bestagon-proto/`](bestagon-proto/) holds 9 de-risk prototypes (hex
math, pathfinding, capture state machine, map generation, fog and AI
perf gates, render gate, screen wireframe, scenario loader) and
[`bestagon-sim/`](bestagon-sim/) holds the shared `combat-core`
resolver plus 5 Monte-Carlo balance sims -- see
[bestagon-sim/README.md](bestagon-sim/README.md), the GDD
([bestagon-design.md](bestagon-design.md)) and the TDD
([bestagon-tdd.md](bestagon-tdd.md)).  All of them run headlessly and
most carry in-source self-tests.
