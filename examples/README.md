...
A self-contained maze generator (~6,600 lines), a homage to Steve Capps' "Amazing" on the 128K Mac.  Ships **eleven** maze algorithms (recursive-backtracker, Kruskal, Wilson, Eller, binary-tree, sidewinder, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree, and recursive-division -- the one wall-adding generator) across **five** grid topologies -- square (the full zoo), plus hex, theta polar, triangle, and upsilon octagon.  Seven of the eleven algorithms (backtracker, Kruskal, Wilson, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree) run on **every** grid through a shared topology descriptor; Eller/binary-tree/sidewinder/division stay square-only -- 35 portable combos.  Renders BFS distance-field heatmaps in ten colormaps with `--solve` / `--braid` / `--weave` overlays, and a `--metrics` quality report on any grid.  **Masking** carves the maze into a shape -- a word (`--mask-text MAZE`, via the built-in 5×7 font), the Trix wordmark (`--mask logo`), or an analytic disc/ring/frame -- with `--mask-invert` to punch the shape out of a full maze instead; disconnected shapes (letters) become one perfect maze per connected piece, and every mask is generated in-process (no bundled image files).  Output is a real PNG whose format is assembled in Trix (no libpng) over the engine's native `deflate` (zlib) and hand-rolled `crc32`/`adler32` ops.  Companion teaching + implementation guide: [amazing-manual.md](amazing-manual.md).  `--self-test` runs 179 assertions; [`gallery.sh`](gallery.sh) renders an 86-PNG showcase (including the full algorithm-by-topology matrix) into `examples/maze-gallery/` (gitignored).  Needs `--vm-size=8M`+ (more for `--stress`); see the manual for sizing.
