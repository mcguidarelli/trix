...
| --- | --- |
| `./build.sh` | Debug build of `./trix` and `./tetrix`: `-O0`, ASan + UBSan, `-Werror`, all optional features (debugger, vm-heap-tracking) enabled.  Silent on success. |
...
| `./build.sh --optimized` | Optimized build, written to `./trix.opt` and `./tetrix.opt`: `-O3`, `-DNDEBUG`, no sanitizers, stripped, `-fhardened`.  The sanitized `./trix` is left in place. |
...
Seven entry points, layered from "everything" to targeted phases:

| Entry point | Covers |
| --- | --- |
| `./runtests.sh` | The full battery: `test_all.trx` master runner, standalone per-test sweep, golden-stdout tests, PTY scenarios, snapshot tests, eqref overflow, sandbox-mode tests, fuzz-regression replay.  This is what CI runs; a crash or missing success marker is a FAIL. |
...
| --- | --- | --- |
| `ci.yml` | push to `master`, every PR | Build matrix {gcc-15, clang-18} x {Debug, Release}; stages the binary and runs `./runtests.sh`, then verifies the README examples still execute and all doc links/anchors resolve.  Separate `clang-format` check job. |
| `clang-tidy.yml` | push / PR on `master` | Static analysis (clang-tidy-20 over the single TU; checks curated in `.clang-tidy`). |
| `codeql.yml` | push / PR on `master`, weekly | GitHub CodeQL security analysis. |
